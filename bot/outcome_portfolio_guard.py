...
class OutcomePortfolioGuard:
    ...
    MAX_DAILY_GROSS_ENTRY = Decimal("200")
    MAX_DAILY_REALIZED_LOSS = Decimal("-8")
    ...
    def evaluate(self, *, outcome_id: int, prospective_notional: Decimal,
        ...
        zero = OutcomePortfolioDecision(True, "portfolio_guard_not_active_before_20_canary", False, Decimal("0"), Decimal("0"), 0, 0)
        if not enabled:
            return zero
        ...
        try:
            ...
            return OutcomePortfolioDecision(False, "portfolio_guard_journal_unavailable", True, Decimal("0"), Decimal("0"), 0, 0)
        ...
        decision = OutcomePortfolioDecision(True, "portfolio_canary_approved", True, gross, realized, consecutive, market_losses)
        if gross + prospective_notional > self.MAX_DAILY_GROSS_ENTRY:
            return OutcomePortfolioDecision(False, "portfolio_daily_gross_entry_cap", True, gross, realized, consecutive, market_losses)
        if realized <= self.MAX_DAILY_REALIZED_LOSS:
            return OutcomePortfolioDecision(False, "portfolio_daily_realized_loss_cap", True, gross, realized, consecutive, market_losses)
        if market_losses >= self.STOP_MARKET_AFTER_LOSSES:
            return OutcomePortfolioDecision(False, "portfolio_daily_market_loss_stop", True, gross, realized, consecutive, market_losses)
        if consecutive >= self.PAUSE_AFTER_CONSECUTIVE_LOSSES:
            ...
            if (datetime.now(timezone.utc) - latest).total_seconds() < self.PAUSE_SEC:
                return OutcomePortfolioDecision(False, "portfolio_consecutive_loss_pause", True, gross, realized, consecutive, market_losses)
        return decision
