...
def report(db_path: str | Path) -> dict[str, Any]:
    ...
    for key, entries in decisions.items():
        for decision_ts, kind, payload in entries:
            ...
            for ts, event in following:
                ...
                if isinstance(timing, dict):
                    for name in (
                        ...
                        "ioc_submit_ts", "ack_ts", "first_fill_ts", "complete_fill_ts",
                    ):
                        try:
                            if name in timing:
                ...
                elif state in {"CLOSED", "EMERGENCY_RESIDUAL"}: marks.setdefault("complete_fill_or_reconcile_ts", ts)
            ...
            ordered_names = [
                ...
                "complete_fill_ts", "complete_fill_or_reconcile_ts",
            ]
    ...
    end_to_end = [
        row["risk_detected_ts_to_complete_fill_or_reconcile_ts"] for row in base["rows"]
        if "risk_detected_ts_to_complete_fill_or_reconcile_ts" in row
    ]
    base["end_to_end_ms"] = _summary(end_to_end)
    if not end_to_end:
        base["blockers"].append("no_complete_instrumented_emergency_exit_chain_yet")
    elif float(base["end_to_end_ms"]["p90"] or 0) > 11_000:
