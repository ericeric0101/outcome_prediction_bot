...
1. **Live P2/P3 capture ownership（完成；2026-09-01 強化）：** `bot/outcome_research_capture.py` 由 `bot.launcher` 在 live mode 以已讀取的雙側 L2 book／本機 receive timestamps 寫入 `OUTCOME_P2_PARITY_SNAPSHOT` v3。P2 public-book snapshot 先同步、立即落盤；同一 wallet 的 `userFills` 與 `userFees` 改為**單一、唯讀 background sync**（fill refresh 至少相隔 15 秒、fee refresh 至少相隔 300 秒），其結果以 `fee_evidence.observed_at_ms/age_ms/account_sync_in_flight` 如實標示。這避免慢速 account endpoint 把 5 秒 book capture 變成約 60–100 秒的 blocking loop；首次 account sync 尚未完成時 snapshot 保留 `pending_first_observation`，不可誤稱已有新鮮 fee evidence。actual fills 仍以 official `userFills` 寫入，P3 照 accepted quote history 產生 1/5/10/30s executable markout。此模組不含 gateway、private key signing 或 `/exchange`；`OutcomeLiveExecutionRuntime` 仍是唯一 order writer。
2. **Heartbeat / gap alerts（完成；2026-09-01 校正）：** live capture 每 60 秒寫 `OUTCOME_RESEARCH_CAPTURE_HEARTBEAT`；gap 的有效門檻為 `max(configured gap, 3 × configured capture interval)`，並寫入 `expected_interval_ms`。因此 alert 表示至少三個預期 samples 缺失，而非單一昂貴 account read 造成的假告警；所有 gap 仍不插值。Binance collector 對應寫 `BINANCE_OI_HEARTBEAT`（60 秒）與 `BINANCE_OI_GAP_ALERT`（90 秒）。設定值位於 `.env.example`：`OUTCOME_RESEARCH_CAPTURE_INTERVAL_SEC`、`OUTCOME_RESEARCH_HEARTBEAT_SEC`、`OUTCOME_RESEARCH_GAP_ALERT_SEC`、`BINANCE_OI_HEARTBEAT_SEC`、`BINANCE_OI_GAP_ALERT_SEC`；全部只影響資料品質監控，**不授權下單**。操作者可用以下唯讀 command 檢查 freshness/gaps：
...
5. **驗證與現況：** 新增 WS→terminal BBO cache、nonblocking slow-account capture、strict durable FIFO fallback 與 S3 使用 durable fallback 的 regression fixtures；定向 tests **44 passed**，完整 Python regression **467 passed**。本次只改善 market-data/display、research cadence 與 emergency cost-basis reconstruction，**不改變** S0 entry threshold、S3 的 2h/20m/-8%/-12% gates、notional limit、或任何新下單權限。部署新碼後，正式運行組合為：一個 Binance OI collector + 一個 launcher live strategy；不要再啟動 `scripts/outcome_shadow.py`。首次運行後必須確認 health report 的兩個 heartbeat 都在 120 秒內、P2 snapshot timestamp 間隔接近設定的 5 秒且 gap alert 不再每次 capture 增加，並確認 terminal BBO 僅在 WS 真正超過 freshness timeout 時顯示 `None`；否則停止新 entry，先修 data path。
...
| **協議結算** | `SETTLING` | 保留 retiring market，官方 SDK 查驗 settlement；不以 BTC/USDC UI 推論 payout，也不自動記錄 `MARKET_SETTLEMENT`。 | 剩餘時間 $\le 0\text{s}$ |
