...
**Production configuration consolidation and reward research (2026-08-30):** The design principle is to minimize `.env` and shell parameters: durable strategy and risk logic must be calculated from verified data, not depend on a fragile manual flag list. `bot.launcher --live` already requires a typed `yes` confirmation and acquires the Outcome-specific single-wallet lock; after that authorization, code enables the official SDK execution, S0 live strategy, and E6 ALO reprice paths directly. The prior four execution gate variables and P3 enable flag are no longer operator settings. `.env` retains only stable local facts and bounded risk values: `$11` entry/exposure, one open order, 55¢ entry floor, journal path, and Outcome lock path. P3 remains dormant unless deliberately developed again; no current live command starts it. The S0 daily-entry cap and environment-configured reprice-attempt cap are removed. This does **not** remove the non-negotiable safety invariants: one live inventory/order, verified account ownership, fresh OI and order book, OI/spot/mark confirmation, 55¢ no-trade band, typed live confirmation, post-only ALO, cancel-confirm-rebook, requote interval/hysteresis, price-bound validation, and fee-inclusive no-loss exit floor. The E6 time tiers remain +5% → +3% → +2%.
