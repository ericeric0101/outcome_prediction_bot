...
**F4/P5 verification。** official sidecar build 通過；persistent child reuse、response identity、exit replacement audit、thesis-aware loss-band、Tier-B thin/wide/depth/drift gate、WS payload scope、既有 runtime/lifecycle regression 共 **72 passed**。maintenance `--report` 在現行 journal 成功讀到 historical allMids 尚存在；它是 storage 維運 evidence，不是 live 策略信號。重啟 launcher 後，應以 `OUTCOME_RUNTIME_TIMING` 的 `account_recovery_ms`、book request、journal write、last SDK request，及 `ORDER_SUBMIT.audit.sdk_submit_timing` 比較 `decision → submit`、`submit → SDK response`、一般 holding loop；未收集前不得宣稱外部 latency 的實測改善值。
