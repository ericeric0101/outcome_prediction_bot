...
**Deferred S0 gate ablation (not yet authorized):** Do not loosen the three-source rule based on UI direction or a single winning daily instance. After the new `OUTCOME_ENTRY_GATE_DECISION` stream has collected sufficient independent daily-market observations, compare current `spot ∧ 5m mark ∧ 5m OI` against pre-registered alternatives such as `spot ∧ mark` and `spot ∧ (mark OR OI)`. Compare entry eligibility, actual maker-fill probability, entry markout, holding time, fee-adjusted realized result and tail loss by daily instance/time-to-expiry; do not credit non-filled hypothetical quotes as PnL. No gate threshold or live entry authority changes until that report is reviewed.
