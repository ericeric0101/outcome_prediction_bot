...
8. **平倉後與重啟後：** official fills／account truth 先完成 FIFO reconciliation，只有確認 flat 且不存在 managed order 才重新從當下 S0/Tier gate 選邊；不沿用前一倉的 side，也不因 loss exit 自動反手。重啟 recovery 只接管有 matching durable audit、market/coin、inventory 與 ownership 的 managed order；未知／手動訂單一律不接管。
