...
|---|---|---|
| **B1 — decision dataset（完成）** | `bot/outcome_active_dataset.py` 由 X3 `outcome_oi_feature_rows` 以每 market/side 60 秒 event-time cadence 建立 compact rows。特徵包含 side mid/spread、距 50%、time-left、side-signed 5m/15m/60m Binance mark 與 5m OI activity；targets 包含 5/15/30/60m executable future bid、離散 horizon MFE/MAE、+1/+2/+5% hit/time、-5/-10/-20% breach/time 與 breach 後 recovery。 | X3 的 path label 只在既有 horizon 觀測，不是連續 tick path；future **bid** 才是 exit evidence。不得把 maker touch 當 fill、把 settlement 勝負當任意時間可成交價格，或把同一 daily market 的相鄰 rows 隨機切入 train/test。 |
| **B2 — multi-target model（完成 shadow artifact）** | `python -m bot.outcome_active_model --db logs/outcome_shadow.db --period 1d --sample-interval-sec 60 --artifact logs/outcome_active_model.json` 以可審計 ridge models 同時估 5/15/30/60m future bid、target/loss/recovery probabilities，並保存 residual q10/q50/q90。validation 按完整 daily market chronological walk-forward；maker fill probability 只由 official BUY submit/fill order id 估計並按 known spread bucket 分開，legacy unknown spread 不套到有 live spread 的決策。artifact 固定 `live_authority=false`。 | 每個 continuous target 必須與同 OOS rows 的 persistence baseline（future bid=current bid）比較；model 未勝 baseline 就是明確 blocker，而不是以 sample 大掩蓋。probability targets 仍須校準、tail/recovery 仍需跨 weekday/weekend/time-left 驗證。更新 artifact 應在 writer 停止或一致性 DB copy 上完成，runtime 只在重啟時載入 frozen artifact。 |
