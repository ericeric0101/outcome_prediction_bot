...
### 2026-09-20 — Outcome DB Retention / Telemetry V1（保守、operator-only）

**完整依賴稽核結果。** V1 已掃描 `TradeJournalDB` schema、所有 SQLite/JSON SQL、runtime/launcher/WS recorder、lifecycle/recovery/reconciliation、全部 committed reports/replays、feature builders、walk-forward/model code、scripts 與 tests。實際 `logs/outcome_shadow.db` 在稽核時為約 **9.4 GiB**（WAL 約 13 MiB）、`freelist_count=0`；有 18 個 table、3,707,153 strategy events、8,545 order events。最大 row families 為 `OUTCOME_WS_L2_BOOK` 743,845、`OUTCOME_ENTRY_ADMISSION_DECISION` 533,155（近期樣本 payload 約 6.9 KiB/row）、`OUTCOME_ENTRY_GATE_DECISION` 531,628、`DERIBIT_FEATURE_SNAPSHOT` 418,807、`OUTCOME_RUNTIME_TIMING` 285,273、`OUTCOME_P2_PARITY_SNAPSHOT` 226,851、`OUTCOME_WS_ALL_MIDS` 223,156、`OUTCOME_WS_TRADES` 113,893。故不可把所有成長錯誤歸因於單一 raw WS type。

**table inventory 與 authority。** `strategy_runs`、`order_events`、`outcome_fill_registry`、`outcome_realized_pnl_lots`、`outcome_market_settlement_registry`、`outcome_settlement_fill_cursor`、`outcome_settlement_payout_fills` 與 `outcome_settlement_status` 是 CANONICAL，永久保留。`outcome_wide_spread_candidates`／`_paths`、`outcome_oi_feature_rows`、`outcome_deribit_feature_rows`、`outcome_oi_fill_feature_rows` 是 DERIVED_DURABLE，長期保留。`binance_oi_observations`（live S0 讀 recent row，X3 也需歷史）、`hyperliquid_perp_context_observations`（native-vs-Binance comparator）、`outcome_p3_quote_index` 和 `outcome_p3_pending_fills` 是 RESEARCH_REPLAY／bounded operational evidence；P3 已有自己的 final-horizon expiry prune，V1 不改它。`strategy_events` 是 mixed table：event family 不明即 `UNKNOWN_DEPENDENCY → DO_NOT_TOUCH_UNTIL_DEPENDENCY_RESOLVED`。

**模型／replay 依賴。** Active B1/B2 dataset/model、fair-value、OI walk-forward 都直接讀 `outcome_oi_feature_rows`；其建構又需要 `OUTCOME_P2_PARITY_SNAPSHOT`、`binance_oi_observations` 和 official `FILL_MARKOUT`。Deribit walk-forward 讀 `outcome_deribit_feature_rows`，其來源為 P2 與 `DERIBIT_FEATURE_SNAPSHOT`。Exit/fast-failure replay 讀 `OUTCOME_WS_L2_BOOK`；stale-cancel entry replay 讀 `OUTCOME_WS_TRADES` 和 P2；holding/market-risk/post-fill/structural-collapse reports 讀其 compact derived events與 FIFO lot evidence。因此 P2、L2、trades、admission/gate decisions、holding/reversal/timing shadows、`FILL_MARKOUT`、`OUTCOME_ENTRY_READINESS_SHADOW`、`OUTCOME_STRUCTURAL_COLLAPSE_SHADOW`、lifecycle、settlement 和所有 order evidence 本版均不可刪。這同樣說明目前沒有把 raw history 刪掉後再補模型的捷徑。
...
**fresh re-arm contract。** stale cancel 不可立即重用原 S0 decision。即使 signal 持續 eligible，entry 一律拒絕；必須先觀察到全域 admission signal ineligible，durably 記為 `OUTCOME_ENTRY_REARMED`，再出現 ineligible → eligible 的新 transition，且新 decision timestamp 必須晚於 re-arm，才可重新走完整 S0、fresh book、portfolio 與 ALO admission。此 state 從 journal 推導，重啟不會遺失；它不是以任意 cooldown 取代新訊號。其 durability linkage 是 exact episode：`STALE_CANCEL_DECISION(id) → STALE_CANCEL_CONFIRMED(stale_cancel_decision_event_id) → DECISION_EXPIRED(cancel_confirmed_event_id) → REARMED(expired_decision_event_id)`；任一當期 link 缺失（包含 confirmed 成功、expiry 落盤失敗後重啟）都 fail-closed，絕不可借用較早 cycle 的 expiry/re-arm。若取消決策後才收到同一 OID 的 immutable official BUY fill，這**不是** zero-fill confirmation，絕不可偽寫 `STALE_CANCEL_CONFIRMED`；僅在後續 fresh account recovery 已證實該 Outcome 的 inventory 與 open orders 均為零時，才可寫入獨立的 `OUTCOME_STALE_ENTRY_FILL_TERMINAL_RECONCILED(stale_cancel_decision_event_id)`，再由 `OUTCOME_ENTRY_DECISION_EXPIRED(terminal_reconciliation_event_id)` 接續同一 re-arm contract。官方 fill、fresh account-flat truth、任一 durable write 任一缺失時均繼續 fail-closed。60 秒 clock 一律採 immutable `ORDER_SUBMIT.ts`，restart adoption 或後續 lifecycle state write 不得重置其年齡。此前已修的 exit-terminal barrier 同時維持：bot-owned protective SELL 尚未 terminal 或一個在其 SELL fill 前評估的 decision 尚未過期時，不得送新 BUY；manual/external SELL 不得被誤採納為 bot exit。
...
| 監控、dashboard、process lock：`monitoring/*`、`dashboard_state.py`、`process_lock.py` | SQLite journal、Outcome shadow dashboard、operations monitor、launcher 的 Outcome terminal dashboard 與 process lock。 | `test_trade_journal_*`、`test_outcome_operations_monitor.py` | **部分移植**。Telegram control/notification plane 已移除；現有 dashboard 對 Outcome 的 order/fill lifecycle 仍應以 journal 為準，不可把 simulation UI 欄位當 live evidence。 |
...
**此審計產生的自動策略 blocker（2026-08-24 歷史優先順序）。** 後續 S0 已提供受限 live entry；下列未完成項仍是擴大風險、宣稱策略 alpha、或啟用 payout/hold-to-settlement 的 blocker，不再是「S0 runtime 一律不可啟動」的描述。
...
4. 取得並持久化 official settlement / payout / fee evidence，完成 standalone outcome 的 PnL、hold-to-settlement 與 rollover 對照。這些未完成項維持 P0/P2/P3 經濟驗收與資金擴張的 blocker；不取代後續已明確記錄的受限 S0 live 權限。

**本切面驗收結果（2026-08-23；歷史起點）**：fixture 已證明相同帳戶快照能產出既有 `PositionState`、餵入 `PositionManager`，並由既有 `ExitPolicyEngine` 產生 `HOLD_TO_REDEEM`。當時配置錢包的唯讀同步仍為空，故原文將 live execution 鎖定。後續已取得非空 inventory/open-order/userFills，並完成主網下單、取消、成交與 lifecycle 回讀；現行 runtime 狀態應以本文件開頭的「實盤狀態」及 S0/E6 為準。`HOLD_TO_REDEEM` 本身仍非 Outcome payout 的自動授權。
...
| P3 — adverse-selection 模型 | 使用真實或明確標為模擬的 passive fill counterfactual，估計與採樣 cadence 相容、且有實際 timestamp 證據的 executable markout、fill probability、cancel race 與 recovery fill probability；按 time-left、side、spread、depth、vol regime 分桶。現行 5 秒 collector 僅接受 5/10/30 秒觀測。 | 每個可啟用 bucket 至少 30 個獨立 fill 樣本；95% 信賴下界的費後 EV 為正。無樣本 bucket 維持不報價。 | 禁止 |
...
2. **P1-2 — venue pre-trade risk gate。** 新 entry 必須通過 `OutcomePreTradeRiskGate`：可用 `USDH/USDC`（`total-hold`）、整數 shares 後的真實名義金額、最多一筆 open order、及以每股 $1 worst-case 評價的 Outcome 庫存曝險。2026-09-07 起正式 $20 canary 的單筆與總曝險上限均為 $20；可用 `OUTCOME_MAX_ENTRY_NOTIONAL_USDC`、`OUTCOME_MAX_OUTCOME_EXPOSURE_USDC`、`OUTCOME_MAX_OPEN_ORDERS` 收緊，但不得自行增加 open-order 數量或提高至 $20 以上。
3. **P1-3 — WS freshness/gap fail-closed（更新 2026-09-03）。** `OutcomeStreamHealth` 要求：連線正常、連線後已完成 REST resync、Yes/No 兩側都收到 L2、且兩側都在 **15 秒**內新鮮。實盤 journal 顯示 Outcome quiet-book 的成對 L2 更新通常相隔約 5–6 秒；原本 3 秒門檻會把正常、仍連線的安靜盤面誤判為 `ws_book_stale`。15 秒容許兩個正常更新區間仍可下新 entry，但超過此時間、disconnect、重連尚未 REST resync、market rollover、任一側 missing 仍 fail-closed。這不阻擋 cancel 或既有倉位的保護處置，避免資料故障反而使風險暴露無法縮小。launcher 會在市場 rollover 重建 recorder，並把狀態交給 official runtime。**事故修正：** 2026-09-03 的 journal 記錄到 14 次 disconnect event（含一次 `1011` ping timeout 後的 10 次以上 opening-handshake timeout）；舊 client 在第 10 次失敗後自行停止 loop，導致 terminal 持續 `ws_disconnected`，只能人工重啟。`OutcomeClient` 現改為 1/2/4/…/30 秒 capped backoff 持續重試、每次重新連線 replay 所有 subscription，並在等待期間清空 closed socket reference；新 entry 仍保持 fail-closed，直到 REST resync 與雙側 fresh L2 都完成。此修正的 unit test 模擬連續 12 次 handshake failure，確認不再產生 `reconnect_exhausted`、不會自行停止。
...
- **P3 — fill/markout calibration（2026-09-05 修正）：** 僅以交易所回讀的真實 `userFills` 建立樣本，或明確標為 `UNKNOWN_NO_QUEUE_MODEL` 的 passive counterfactual；不得把後者當真實成交。現行 5 秒 collector 的 v2 P3 每筆真實 maker fill 只測量 **5/10/30 秒** target，且必須取得距 target **±2.5 秒**內的 executable opposite-side bid/ask；每筆保存 `actual_elapsed_ms`、`target_lag_ms`、quote timestamp 與 snapshot id。1 秒觀測需要獨立、經驗證的 ≤1 秒 capture path，未建立前不得偽造 1 秒 label。缺 quote／超過容忍窗就是 missing evidence，不合成 loss、也不拉長標成其他 horizon。資料需按週期、time-left、side、spread、depth、volatility regime 分桶，且每個可能啟用的 bucket 至少 30 個**獨立真實 fills**，95% 信賴下界的**observed-fill-fee-adjusted** EV 為正；它仍不是 final round-trip PnL，因為假設出場的未來 fee/滑價另須記錄。1d 與 15m 永不共用 calibration；15m 上線後從零開始收集。
...
4. **P3-M2（v1 歷史實作；已由 v2 取代）：** v1 曾從 journal 的 P2 raw L2 snapshots 以同一 coin 的首個 future executable quote 產生 1/5/10/30 秒 markout；BUY 使用 future bid，SELL 使用 future ask。2026-09-05 已證實此「首個晚於」規則會在 capture gap 時錯標 horizon，故 v1 rows 保留 audit、不可進 report/gate/X3 overlay。現行規格以本文件的 P3 v2 exact-tolerance contract 為準。
...
4. **P3 actual-fill collection（v2 修正完成；樣本驗收未完成）：** `OutcomeExecutionLedger.sync_fills()` 把每個 official `userFills` 回讀記為 `actual_fill=true`、`period`、`fill_provenance=hyperliquid_userFills`。`P3_MARKOUT_SCHEMA_VERSION=2` 只從 accepted v3 P2 quote history 對同週期 confirmed maker fill 寫入 5/10/30 秒的 exact-tolerance executable markout；舊 v1 的「首個晚於 horizon 的 snapshot」可把 31 秒／63 秒後 quote 誤標為 1/5/10/30 秒，保留供審計但已被正式 P3 report、research gate 與 X3 fill overlay 排除。v2 重新從零累積每 bucket ≥30 independent actual maker fills。
...
**本輪驗證（2026-08-25；歷史研究 gate）：** P2 data-quality/report/P3 replay/client tests **15 passed**；P3 fill-provenance/research-gate/live-runtime tests **12 passed**；完整 Python regression **387 passed**；official SDK sidecar `npm run build` 通過。當時下一步是重新啟動 collector 以產生 schema v3 rows、取得 per-market conversion/settlement evidence，並在受限授權下累積 maker fills。P2/P3 至今仍未完成經濟驗收；然而其後已另行啟用受限 S0 live runtime，兩者不可混為「策略已證明盈利」或「runtime 尚不可下單」。
...
**P3 校準 canary 與操作者操作順序（2026-08-25；歷史 calibration 模式）：** 純 read-only shadow 不可能產生 actual fill，故不能單獨完成 P3。當時新增的「P3 校準 canary」是收集執行成本證據的正式受限 runtime，**不是** P4 strategy canary：它不使用方向預測、不因 `SignalEngine` 入場、不把結果當策略獲利、也不能解除 `OutcomeResearchGate`。其唯一目的是在明確操作者授權下以真實、低額、post-only maker lifecycle 產生可審計的 fill／markout／退出／payout evidence。**此段的「不得直接開啟策略／預設停用」只描述當時 P3 模式；現行 S0 live strategy 的啟用方式與限制以 S0/E6 段落為準，並未因此宣告 P2/P3 經濟驗收完成。**
...
**本輪 P2/P3 runtime 驗證（2026-08-25；歷史快照）：** P3 consensus-entry / ±5% maker-exit policy、maker state machine與 live execution gate 的針對性 regression **18 passed**；完整 Python regression **394 passed**；official SDK sidecar `npm run build` 通過。當時新增 `.env.example` 的 calibration defaults 均為 disabled，尚未啟動 live runtime、未提交 exchange action；這已被後續主網 lifecycle 驗證與 S0 live 接線取代。本輪也修正 HIP-4 實際帳戶 inventory 為 `+<id>` 而 order book coin 為 `#<id>` 的表示差異，正式 state machine 現在會接受兩者並以 wallet inventory/`entryNtl` 計算保護性 sell，不會因 token prefix 遺漏已成交庫存。
...
3. **主動 maker sell cancel/replace（2026-08-26 的歷史待辦；E0–E5 已完成基礎 infrastructure）：** 當時 runtime 只會在初始 fee-adjusted take-profit 後維持原價，且舊 loss-band 取消沒有完整 confirmation/readback。後續 E0–E5 已將 ownership、single-flight、cancel confirmation、inventory/fill race readback、fresh L2 rebook、ALO non-crossing、hysteresis 與 durable lifecycle 接入正式路徑，並有主網 E5 one-shot 證據。**尚未完成的不是這套 infrastructure，而是何時應作 profit improvement／risk reduction 的資料支持 policy；該 policy 工作現納入 S1/S2，並且仍禁止以立即止損名義跨 bid 或 fallback 為 taker。**
...
**現況與缺口（2026-08-28 前的歷史描述；已由 E0–E5 取代）。** 當時 `OutcomeMakerStateMachine` 尚未確認取消後的 official open-order truth、未在取消後重新讀 L2、沒有 replacement lock／durable partial-fill 與 cancel/fill-race lifecycle。這些 execution infrastructure 缺口已由 E0–E5 補齊。現行仍不可把 -5% ALO loss-band 誤解為止損保證：未完成的是 S2 的 holding-path evidence、reversal policy 與 loss-fill 後 re-entry discipline，而不是 cancel-confirm-rebook 的基本正確性。
...
| **E4 — disabled production gate（歷史設計；已由後續 S0 接線取代）** | 初始設計為將 E0–E3 接入正式 `OutcomeLiveExecutionRuntime`，但預設 `OUTCOME_EXIT_REQUOTE_ENABLED=0`；後續 `launcher --live` 已在 typed `yes` 後於程序內啟用正常 E4 re-quote，E5 canary 仍關閉。 | full regression、SDK sidecar build、live-like fake account test；啟用 flag 缺失時斷言零 cancel/replace。 | 初始版本預設禁止；現行權限以 S0/E6 為準。 |
...
**E0–E3 實作與驗證（完成 2026-08-28；不改變下單權限）：** 下列是各模組在當時的隔離驗證證據；其中 E2 的 runtime 接線狀態已由後續 E4／S1 正式路徑取代，應以本文件較後方的「目前 S0 live 語意」為準。
...
3. **E2 — cancel-confirm-rebook-replace controller（完成；當時尚未接入 runtime）：** 新增 `bot/outcome_exit_requote_controller.py`。controller 先寫 `CANCEL_SUBMITTED`、提交既有 ownership-checked cancel、重讀 account open orders 確認舊 `oid` 消失、重讀 inventory 檢查 cancel/fill race，再讀 L2 並以 `price > best_bid` 的 tick-aligned ALO reduce-only sell 送 replacement。任何 cancel timeout/reject、old order 仍在、inventory 改變、book 無效/crossing、或 replacement reject 都只記 `RECONCILE_REQUIRED`，不補送單也不走 taker。**E0–E3 完成當時尚未被 `OutcomeLiveExecutionRuntime` import 或呼叫；後續 E4／S1 已接入正式路徑。**
...
5. **測試與安全界線：** E0 targeted 加既有 maker/gateway regression **19 passed**；E1 targeted **9 passed**；E2 targeted **21 passed**；E3 新增 suite **14 passed**；完整 Python regression **423 passed**。新增測試包括：fee floor、tick/passivity、stale book、hysteresis/interval/attempt cap、restart ownership、unrecorded order、cancel-confirm ordering、partial fill/cancel race、invalid rebook、replacement reject、period isolation 和 `execution_submitted=false`。當時沒有提交 Outcome cancel/order，也沒有把 `OUTCOME_EXIT_REQUOTE_ENABLED` 寫入 runtime；這一前置限制已在後續 E4／S1 integration 中取代。

**E4 實作與驗證（完成 2026-08-28；其後由 `launcher --live` 正式接線）：**

1. `OutcomeLiveExecutionRuntime` 現在持有 E0 planner、E1 lifecycle store、E2 controller。初始 E4 canary 要求四個 gate；現行 `launcher --live` 在使用者鍵入 `yes` 後，會在程序內啟用 automated execution、official SDK execution、S0 live strategy 與 E4 exit re-quote。`OUTCOME_EXIT_REQUOTE_CANARY_ENABLED` 仍預設關閉，不能由一般正式啟動命令自行打開。
...
**S0/E6 — 受限 OI live strategy 與時間型被動減損出場（2026-09-06 校正）：** S0 僅在 fresh Binance BTCUSDT public OI、同方向 Binance mark return／OI change、及 BTC spot 相對 Outcome strike 的方向確認同時成立時，才選 YES/NO；Tier-B 例外僅放寬 OI 方向，仍要求 fresh OI/mark、book quality 與其他風控 gate。選定側 first-level maker bid <55¢ 時不可進場。出場先採 fee-after data-derived target（1%–5%，資料不足時 3%）；1h 後最多收窄至 +3%，2h 後最多收窄至 +2%。**-5% midpoint breach 不再單獨授權 loss-band：還必須同時有已滿 2h 的 tier、fresh BBO、持續三次的 opposite spot/mark/OI reversal classifier、及仍有效的 -5% loss threshold。** 任一條件不成立（尤其同方向確認重新出現）即撤回 loss-band 資格，恢復當前 +2% passive target。它不是已成交的 stop，不會跨 bid；S3 的 price-protected IOC 條件亦未放寬。flat 後下一 entry 重新以 S0 當下條件決定 side。單筆／單 market 仍受 $11、單一 open order、ownership、freshness 與 ALO 限制，結果不得外推為 alpha 或擴大資金授權。
...
**Production configuration consolidation (校正 2026-08-31):** 設計原則仍是最小化 `.env`／shell surface。`launcher --live` 在 typed `yes` 及 Outcome-specific single-wallet lock 後，於 process 內啟用 SDK execution、S0 與 E4 requote；P3 不由正常 live command 啟動。`.env` 仍保留穩定本機風險與策略常數，包含 $11 entry/exposure、單一 open order、55¢ entry floor、journal/lock path，以及 S0 target／1h／2h tier；它們不是每次啟動必填的 shell 參數。daily-entry cap 與環境式 replacement cap 已移除。不可移除的不變量包括單一 inventory/order、ownership、fresh OI/book、S0 confirmation、ALO、cancel-confirm-rebook、hysteresis、price bounds，及**時間與確認雙條件的被動 -5% loss-band（不是 no-loss floor）**。

**Redis removal (2026-08-30):** Redis is not an Outcome performance, telemetry, or coordination dependency. All Redis initialization/preflight warnings, `REDIS_*` runtime-environment handling, legacy strategy constructor plumbing, local `.env` entries, and the Python package dependency have been removed. SQLite `TradeJournalDB` / execution journal remains the sole local audit and research store; the single-wallet filesystem `ProcessLock` remains the sole process-coordination mechanism.

**SQLite journal storage lifecycle (2026-09-05):** `logs/outcome_shadow.db` is the single combined Outcome execution/research journal, not a disposable shadow cache. A storage audit found the active file at **4,296,867,840 bytes (4.00 GiB)**, with a 41,232-byte WAL and 32,768-byte SHM; `PRAGMA freelist_count=0`, so `VACUUM` would consume time and temporary disk but would not materially reduce it. `trade_journal.db` is only 48 KiB. The two historical, verified pre-duplicate-fill-repair rollback copies (`outcome_shadow.pre-fill-dedupe-20260826.db`, 782,880,768 bytes; `...20260828.db`, 1,576,558,592 bytes) and a 106,496-byte temporary PnL test DB were permanently removed after confirming no launcher or OI collector writer was running, releasing **2,359,545,856 bytes (2.20 GiB)**. Do not manually delete a live journal's `-wal`/`-shm` files. Before any future repair, first stop every journal writer, make a rollback copy only for the verification window, record it here, and remove/archive it off-machine once the repair is verified; do not retain multi-GiB local pre-repair copies indefinitely. Lossless retention reduction of the active raw P2/WS history requires an explicit, separately reviewed aggregation/archive migration—never an ad-hoc deletion of rows needed by P2/P3/S0 audits.

**Outcome incentive handling (research status, 2026-08-30):** The operator supplied an incentive description allocating per-market rewards among quotes, maker fills, and taker fills. Public Outcome documentation confirms that maker/taker roles and fees are venue concepts, but the currently discoverable official docs did not expose the campaign's complete active eligibility, scoring cadence, qualifying-market list, anti-abuse, or disqualification terms. Therefore the reward is **not** booked as PnL or used to relax any risk limit. Current S0 can contribute only when its resting ALO orders actually qualify as maker/quote activity; it is not a two-sided liquidity-reward market maker. Before creating a reward-aware quoting strategy, record the official campaign terms/market identifier in this document, read the UI's realized reward ledger, then build a separate reward attribution table: actual maker/taker fills, quote uptime/score, fees, adverse-selection markout, realized PnL, and reward actually credited. Any implementation must reject self-trading/wash-volume behavior and must remain profitable after fees and markout even if rewards are zero. Official context: [Outcome fees](https://docs.outcome.xyz/leverage/fees) and [Outcome market concepts](https://docs.outcome.xyz/concepts/markets/markets).

**S1 — data-derived target, time-budgeted passive loss policy, and capital-efficiency telemetry (completed 2026-08-30):** Fixed +5% targets created an unacceptable 1d capital-lock tail: a chronological diagnostic found 39 buy-to-next-sell pairs with a 1.58h mean and 21.28h maximum; the new FIFO report, which avoids double-counting unmatched fills, finds 35 completed round trips, 19 in 5–30m but 2 above 6h. `bot/outcome_exit_target_policy.py` now reads only accepted, same-market P2 L2 snapshots and estimates the median absolute 5-minute executable midpoint move over the last two hours. It selects a fee-after target clamped to **1%–5%**; if fewer than 12 independent observations are available it explicitly falls back to 3%, rather than pretending the tape is informative. This changes the target only, not whether a valid OI/spot/mark entry signal may enter; the only feasibility gate remains the real Outcome price ceiling. The exact target, estimator source, estimated movement and sample count are durable fields of `OUTCOME_LIVE_STRATEGY_ENTRY_PLACED`, so every exit can be audited from the fill VWAP.

**S1 time exit contract:** First sell remains the data-derived fee-after target and is always ALO/post-only. At 1h it can only narrow to the lesser of the selected target and +3%. At 2h it can only narrow to the lesser of that target and +2%; only then may a weak book trigger a fee-inclusive **-5% passive loss quote**. The trigger requires both elapsed time and midpoint at least 5% below fill VWAP, not a one-tick fluctuation. It remains a passive target, not a guaranteed stop: if the whole book gaps below -5%, an ALO at -5% will rest and capital can still remain locked. A guaranteed -5% loss cap would require a taker/marketable order, which remains forbidden. Cancel-confirm-rebook ownership, fresh L2, rate-limit/hysteresis, price bounds and no-crossing checks remain mandatory; there is no arbitrary replacement-count cap.

**S1 report and verification:** `python -m bot.outcome_holding_time_report --db logs/outcome_shadow.db --period 1d` is the formal read-only report for FIFO completed round trips, holding-time bins, mean/max holding duration and fee-adjusted realized return. It does not infer missing fills or submit/cancel orders. New tests cover a quiet 0.2% tape clamping to the 1% target floor, insufficient-tape 3% fallback, FIFO holding-time attribution, S0 entry policy persistence, two-hour -5% eligibility and existing cancel/rebook safety. Targeted tests **24 passed**, full regression **442 passed**, `py_compile` and `git diff --check` passed. This implementation deliberately does not claim P2/P3 economics are complete: their conversion/settlement fee evidence and 30-independent-fill positive-LCB gates remain blockers for increasing size.

**S1 entry-target provenance repair (completed 2026-09-02):** An audit of the 2026-09-01 `#13131` trade found its actual `ORDER_SUBMIT`/fill evidence but no queryable `OUTCOME_LIVE_STRATEGY_ENTRY_PLACED` target-decision record, so the approximately fee-after +1% target could only be inferred from its resting sell. New S0 entries now copy the complete decision into the **same durable BUY `ORDER_SUBMIT` audit payload as the accepted exchange order id**: schema version, policy kind, selected target return, estimator source, estimated 5-minute move (or explicit absence), sample count, maker-close fee, decision bid, clearly labelled decision-time target preview, decision time, and the 1h/2h/loss-tier contract. The later strategy event remains a research index but is no longer the sole audit source. Recovery first reads the strategy event and cross-checks its target/fee against the matching `venue_order_id` submit audit when both exist; if a process dies in the post-acceptance/pre-strategy-event window, it may recover only from a dedicated, same-market/coin S0 submit audit. Manual buys, generic orders, mismatched records and malformed evidence remain fail-closed. Targeted runtime/ledger regression **26 passed**, including deletion of the strategy event to prove the exact order audit retains recovery evidence; `py_compile` and `git diff --check` passed. This is an observability/recovery repair only: it does not change target selection, entry permission, ALO behavior, size or any live-order authority.

**S1 entry quote lifecycle + gate-attribution repair (completed 2026-09-02):** The 9/3 daily instance showed that a later S0-confirmed maker BUY could remain resting for hours; because the account invariant is one open Outcome order, it correctly prevented all further entries but made UI trade history look like the bot had not attempted another entry. The cause must not be conflated with the three-source gate. `OutcomeEntryLifecycleStore`, `OutcomeEntryQuotePlanner` and `OutcomeEntryRequoteController` now provide the formal equivalent of the exit lifecycle for **S0-owned BUYs only**. A new S0 BUY receives `BUY_RESTING` ownership only from its schema-versioned submit audit; after restart the controller may adopt only an exact matching `ORDER_SUBMIT` / `oid` / coin / BUY side / decision-price record, never a manual or generic order. After a fixed **5-minute** minimum age, a resting buy is kept unless a fresh S0 evaluation explicitly loses confirmation, flips side, or has a materially different same-side first-level bid (at least two ticks). Missing/stale data never causes cancellation. A cancel is one mutation: ownership + zero inventory → cancel → official open-order absence + zero-inventory readback; only the **next** tick may re-run S0, risk, fresh book and submit a replacement ALO. Fill/cancel races, changed inventory, missing ownership, unusable book and cancel failure are `RECONCILE_REQUIRED`; no automatic rebook or cross/taker order is sent. Every live-strategy tick now records a compact `OUTCOME_ENTRY_GATE_DECISION` with all three input evidence, decision reason and active inventory/order state, even when no order is sent. This is the dataset required to distinguish gate rejection from a resting-order lock. Planner/store/controller/runtime tests cover interval, stale-feed keep, direction change, hysteresis, audited-only adoption, cancel confirmation, no same-tick rebook and durable decision capture; full regression **473 passed**, `py_compile` and `git diff --check` passed. Existing manually placed or unaudited resting buys remain unowned; a live process must be restarted to load this code.

**S1 filled-entry exception（完成 2026-09-05）：** 上段「next tick」只適用於 **zero-inventory** 的一般 entry re-quote，禁止在同一輪撤買後重新買入。對 `inventory > 0 + exact owned BUY` 的已成交殘單，現行流程改為同一 runtime invocation 內 cancel-confirm → inventory reread → fresh REST L2 → fee-after reduce-only protective ALO sell；它不是重新開倉，也不等待下一個主策略 tick。完整規格與回歸證據見 F1 protection-gap repair。

**S1 final admission telemetry + pre-registered gate-ablation observer（完成 2026-09-03；延遲隔離更新 2026-09-17；不改變 live entry authority）。** 單有 `OUTCOME_ENTRY_GATE_DECISION` 只描述三方原始訊號，無法區分「訊號成立但被 WS、55¢ no-trade band、account/risk、target ceiling 或 existing order 擋下」與「根本沒有訊號」。`OUTCOME_ENTRY_ADMISSION_DECISION` 記錄 raw signal/evidence、active account/recovery、market-data status、selected coin/best bid、no-trade/target/risk gate inputs、final state/reason、是否真的送單與 exact oid。它與所有 regime/confidence/challenger shadow event 都是**best-effort telemetry**：SQLite writer 被其他 collector 持鎖時最多等待 50ms，可能遺漏單筆 observation，但絕不可把 1.5 秒 execution lane 卡住 10 秒。durable order intent、official fill、lifecycle ownership、reconciliation 與任何 fail-closed mutation evidence 維持原本可等待且不可降級的寫入路徑。runtime 同時記錄 research-shadow 各 observer 的毫秒耗時，日後 `OUTCOME_RUNTIME_TIMING` 可區分 DB contention 與 observer compute。`OutcomeOiEntryGate` 同時把三個**預先註冊、telemetry-only** variants 寫入同一份 raw evidence：現行 baseline `spot ∧ 5m mark ∧ 5m OI`、候選 `spot ∧ 5m mark`、候選 `spot ∧ (5m mark OR 5m OI)`；OI 在 DOWN 時仍只代表 activity，方向仍由 spot/mark 決定。runtime 仍只會採用 baseline，variants 絕不授權 order。
...
**S2 的目的。** 將「市場可能反轉」從直覺變成可稽核狀態，並避免 loss exit 後同一 tick 無差別重進。S2 不會在未明確授權 taker 前製造保證 -5% stop；ALO-only 世界沒有 guaranteed loss cap。預設也**不**會在 -5% quote 未成交時自動一路下調至 -10%／-20%，因那只是在沒有成交保證下擴大可接受損失。

| Milestone | 程式修改範圍 | 驗收與 live 權限 |
|---|---|---|
...
| **S2-3 — durable post-loss re-entry（修訂完成 2026-09-17）** | 只有 managed lifecycle 已由 account truth 確認 inventory=0、owned sell 不在 open orders，且同一 `venue_order_id` 具有 official `userFills`-derived `ORDER_FILLED` 時才可評估。進一步必須將該 SELL 與其前一張 bot-owned entry order 的完整 official BUY fills 精確配對，並以 `SELL proceeds − SELL fee − (BUY notional + BUY fees) < 0` 驗證 fee-inclusive realized loss，才寫 `OUTCOME_LOSS_EXIT_CONFIRMED`。`REVERSAL_CONFIRMED` 只是風險狀態，不是損失證據；即使 lifecycle 曾標示 reversal，若最終賣出仍有淨利，絕不可啟動 post-loss guard。讀取既有 v1 loss event 時也會以同一 immutable fills 重算；只有能證明為非虧損者會被忽略，其餘缺資料／無法判定的 legacy event 維持 fail-closed。一次 verified loss 不再封鎖整個 daily market：固定 **15 分鐘**冷卻後，每一次新 entry 仍須通過當下 S0/Tier direction、fresh book、fee-after target headroom、risk 與 ALO gate；若重做相同 coin，新的 decision bid 必須至少收復該 official loss exit price。不同 coin 也必須有當下相反／新方向的完整 S0/Tier signal，絕不自動反手。official SDK 接受 resting BUY 時會 durable 記錄 `OUTCOME_LOSS_REENTRY_SUBMITTED` 作為 audit，但它**不是次數額度**：未成交的 ALO cancel/requote 或後續獨立合格機會均不可因此永久封鎖。舊 S2-3 event 缺 loss price 時只可由其引用的 immutable official fill 讀回，讀不到則 fail-closed。 | 防止 loss 後同 tick 追進，同時避免「曾偵測 reversal、實際卻獲利平倉」被錯當 loss；不把未成交 passive attempt 誤當成已用掉一整個市場的機會。固定常數而非 `.env` 人工調參；不放寬 size、ALO、S3 或一般方向 gate。 |
...
上述 E5 one-shot gate 是歷史 canary 證據，不是目前的正式啟動設定。現行 `launcher --live` 在 typed `yes` 後於程序內啟用 automated execution、official SDK execution、S0 live strategy 與 E4 re-quote；`OUTCOME_EXIT_REQUOTE_CANARY_ENABLED` 預設仍為 `0`，且一般正式命令不得將其打開。E5 僅對當時 process 新建立的 lifecycle 有效；重啟、manual/legacy sell、unrecorded order、無 inventory、或任何 account mismatch 都不具其 one-shot eligibility。正常 runtime 不應再以「兩個 reprice flags 都為 0」描述；現行正式限制是：E4 正常 re-quote 可用、E5 canary 不可用。
...
| **X4 — market-only baseline 與 purged walk-forward（研究程式完成 2026-08-30；證據未通過）** | `bot/outcome_oi_walkforward.py` 只比較兩層可解釋 ridge model：A=`Outcome 自身可成交 YES book 狀態 + decision-time time-left`；B=`A + Binance OI/context`。B 具體加入 5m、15m、1h BTC mark return、OI return、5m-vs-15m acceleration、price/OI divergence、1h OI z-score 與 taker imbalance。目標是同一 Outcome 的未來 5m **executable YES long markout**，不是 mid、不是推論 settlement。train/test 依完整 daily market instance 的時間排序切分，禁止 random snapshot split；每個訓練 market 的最後 5m label overlap 會 purge。 | `scripts/outcome_oi_walkforward_report.py --db logs/outcome_shadow.db` 只輸出 out-of-sample RMSE/MAE、A→B improvement、每 fold 的 purged rows 與 blockers。預設至少 5 個獨立 daily instances、每 fold 100 train／20 test rows、總計 200 OOS rows；資料不足、沒有正向 OOS improvement 或任何缺特徵皆 fail-closed。B 必須在未見過的 daily instances 對 A 呈現可重複增量；交易層仍須另有全部已驗證 fee、slippage、fill/exit cost 後 EV 與 95% LCB。 | **研究-only，永不直接授權 X5 或策略實盤。** 不以 in-sample correlation 解鎖任何 bucket。 |

**X4a — running-market intraday provisional diagnostic（完成 2026-08-30）：** 為避免必須等五個完整 daily markets 才能看到第一個研究數字，`scripts/outcome_oi_walkforward_report.py --intraday` 在**同一 1d Outcome contract**中，分別對未來 **5m 與 15m** executable YES-long markout 做 rolling OOS 比較。它沿用同一 A/B feature set，且只抽取每一 label horizon 一筆不重疊 decision（5m 或 15m），以 30 分鐘時間 block 的 rolling train→test、test 前完整 horizon embargo 做防洩漏。provisional 門檻是 18 train、3 test、18 OOS rows，讓 5m 診斷可在連續資料約數小時後開始產生，而不是等待五日。這裡的「15m」是 **label horizon**，不是宣稱 Outcome 已存在 15m market。因 samples 同屬相同 daily instance、仍有 regime correlation，報表固定 `provisional_only=true`、`ready_for_x5=false`；即使暫時顯示 OI improvement，也只能作為下一輪特徵診斷，絕不可調高下單額度、放寬 entry gate，或取代正式 X4 的跨 market instance 驗收。
...
`X1 → X3` 與 **X4 research implementation** 已完成；X4 的經濟／alpha 驗收尚未通過。新 captures 現在在 P2 snapshot 寫入 `expiry`、`strike` 與 decision-time `time_left_sec`，X3 feature schema 升為 v2；因此舊 rows 缺少該事實，不能偷以現在時間回填。先重啟 launcher、讓同一 market instance 產生新的 v2 snapshots，然後執行 `scripts/build_outcome_oi_features.py --db logs/outcome_shadow.db` 與下列 X4 report。X4 在收足至少五個完整 daily market instances 前必定回報 blockers，這是正確的 fail-closed 結果，不代表 OI 有或沒有 alpha。X3 只完成可稽核資料與 labels，不代表 OI 有 alpha，也不解鎖交易。每個 milestone 都需先更新本文件、加入測試並通過完整 regression，才可進下一步。X2 collector 的正式啟動命令如下，預設對同一 journal 做 500 筆 5m backfill，之後每 30 秒保存一筆 current OI：
...
X3 是離線建構，沒有網路或交易呼叫；同一 schema/source snapshot 會 upsert。**但 build 是會寫入 derived table 的 SQLite 工作，不得在 live launcher 或 OI collector 仍寫同一 journal 時執行**；必須先停止全部 journal writers，或對已 checkpoint 的一致性 DB copy 執行：

```bash
./.venv/bin/python scripts/build_outcome_oi_features.py --db logs/outcome_shadow.db --batch-size 500
./.venv/bin/python scripts/outcome_oi_feature_report.py --db logs/outcome_shadow.db
./.venv/bin/python scripts/outcome_oi_walkforward_report.py --db logs/outcome_shadow.db
./.venv/bin/python scripts/outcome_oi_walkforward_report.py --db logs/outcome_shadow.db --intraday
```

三者皆為 venue-read-only 的研究入口；feature build 只會寫入可重建的本地 derived SQLite rows，兩個 report 只讀 DB。三者都不會啟動 Outcome 或 Binance 下單、取消單，亦不改變 launcher 的 live 權限。

**X3 incremental-build repair（2026-09-11）。** 原 builder 對每個 P2 snapshot 線性掃描全部 OI rows、為每個 label 再掃描 future snapshots，並逐 row 開 SQLite connection/commit；長 journal 因而呈現近似平方時間與無進度輸出，按 `Ctrl-C` 後只留下有效但不完整的 derived rows。現行 builder 對 local-receive OI 建立 binary-search / prefix-sum index、對每個 market 的 label 建立 timestamp index，並以預設 500-row short transactions checkpoint；正常重跑只補 current schema 的缺 row 及最後 60m label window，明確 `--include-backfilled` 或 `--rebuild` 才全量重算。每批於 stderr 印出進度，任何中斷保留的 batch 都可安全續跑。這只修復研究建置效率與可恢復性；不讓 X3、fair-value artifact 或 OI 取得 live entry authority。
...
### P0 data-integrity repairs（2026-09-05：capture cadence 與 canonical PnL 修正完成；live 證據待累積）

1. **Live P2/P3 capture ownership（修正完成；2026-09-05）：** 實際 DB 曾顯示設定 5 秒、但 snapshot 間隔約 **167–256 秒**，且每次都產生 gap alert；先前僅把 `userFills`/`userFees` 放背景 thread 並不足以解決，因為 `launcher` 主交易迴圈仍提供 capture 的 L2 books，會被 wallet reconciliation、SDK／REST retry 與 execution tick 阻塞。現在 `bot/outcome_research_worker.py` 是**唯一** P2/P3 snapshot cadence owner：它用獨立 read-only `OutcomeClient`、`ttl_sec=0` 的雙側 L2、自己的 worker thread 呼叫 `OutcomeResearchCapture`；launcher 只提供當前 market spec，不再寫 snapshot。worker 不 import gateway／sidecar execution、沒有 submit/cancel 方法、不得成為 wallet writer。`userFills`／`userFees` 仍是單一 background read sync，且 snapshot 中如實保留 `fee_evidence` freshness。正式重啟後才會開始產生此版本的實盤 cadence 證據。
2. **Heartbeat / gap alerts（修正完成；2026-09-05）：** capture 每 60 秒寫 heartbeat；gap threshold 仍為 `max(configured gap, 3 × interval)`，但現在它量測獨立 worker 的實際完成時間，而不是交易 loop 的延遲。worker book-read 失敗會獨立寫 `OUTCOME_RESEARCH_CAPTURE_WORKER_ERROR`，不插值、不以舊 book 偽造 snapshot，亦不改變 entry authority。Binance collector 對應寫 `BINANCE_OI_HEARTBEAT`（60 秒）與 `BINANCE_OI_GAP_ALERT`（90 秒）。設定值位於 `.env.example`：`OUTCOME_RESEARCH_CAPTURE_INTERVAL_SEC`、`OUTCOME_RESEARCH_HEARTBEAT_SEC`、`OUTCOME_RESEARCH_GAP_ALERT_SEC`、`BINANCE_OI_HEARTBEAT_SEC`、`BINANCE_OI_GAP_ALERT_SEC`；全部只影響資料品質監控，**不授權下單**。操作者可用以下唯讀 command 檢查 freshness/gaps：
...
**P3 v2 首次部署後 DB 稽核（2026-09-05）：** v2 目前僅有兩筆可接受 observation：5 秒樣本 elapsed **5,239 ms**（lag +239 ms）及 30 秒樣本 **29,660 ms**（lag -340 ms），均在 ±2,500 ms window，且 `(fill_id, horizon_sec, schema=v2)` 重複數為 0。這確認「過晚 quote 偽標短 horizon」已修正；兩個 bucket 各僅 1 樣本，P3 report 仍正確 `ready=false`，不可推論 edge。另一方面，雖 `.env` 設定 5 秒 capture，最近 DB 記錄到 **1,402** 個 gap alerts、相鄰 capture gap 為 **51.1–72.0 秒**（設定 interval 5 秒、alert threshold 30 秒），故 5/10/30 秒的 missing rows 是正確缺失，不能補插。根因是 capture path 每次都全表讀取／Python 解析歷史 P2 snapshots 與 actual fills；journal 已超過 69 萬 strategy events，research worker 因此無法維持 5 秒 cadence。下一個 P3 code work 必須改為 indexed/incremental quote window 與 pending-fill state，而非每 tick replay 整個 journal。另，部署後對舊 fill 補寫的 v2 row 可供 timing audit，但其 time-left/spread/depth bucket 目前取補寫當下 context，未完成 fill-time context reconstruction 前不可用於 regime-bucket calibration。
...
**F1 protection-gap repair（完成；2026-09-05）：** 同一筆 Tier-B BUY 官方 fill timestamp 為 13:10:34 UTC，但第一張 verified ALO SELL 到 13:16:18 UTC 才提交，曾出現約 **5 分 44 秒**無保護 sell 的窗口。根因是 account snapshot 同時看到 inventory 與仍開放的 audited entry BUY 時，舊 controller 回傳 `inventory_present_before_entry_cancel`，使該輪無法繼續至初始 protective sell；不能因該筆最後獲利就把它視為 pass。現行 `OutcomeEntryRequoteController.execute_cancel_after_fill()` 僅對 exact audit-owned BUY 生效，流程為：**inventory + 唯一 owned BUY → cancel → official open-order absence confirmation → inventory readback → fresh account reconciliation → 同一 runtime invocation 的 fresh REST L2 → verified fill-VWAP、fee-after、reduce-only ALO SELL**。它優先於 holding-path/S3 等非必要 read，避免慢觀測延長保護窗口；不需要、也不等待「下一個」全域策略 tick。cancel failure、仍見 old order、inventory 消失／變動、未知 ownership、殘留衝突 order、缺少 persisted exit policy 或 REST book 失敗一律 fail-closed，絕不新買、猜測取消或改成 taker。partial fill 和 cancel/fill race 以取消後的 account truth 重新 sizing。Tier-B restart adoption 亦修正為接受 `s0_spot_mark_tier_b` audit，避免重啟後誤報 unrecorded ownership。新增 controller/runtime regression 覆蓋 cancel → fresh book → reduce-only sell 的同輪順序；完整 suite **490 passed**。
...
1. **Characterization safety tests。** 先固定既有 precedence：protective SELL 必須先於 holding research；保護處理未完成即短路；fast-failure 成立即不得繼續至 S3／entry requote；stream gate 後仍維持 exit requote → persisted exit；account recovery unsafe 必須在新 entry construction 前 fail-closed。
2. **Holding supervisor。** `OutcomeHoldingSupervisor` 專管既有 inventory 的順序、5 秒 reversal 與 30 秒 holding capture cadence、fast-failure、S3、entry remainder requote 與 persisted exit。觀測 cadence state 已由 runtime 移入 supervisor；它不自行建立新的下單 primitive。
3. **Entry supervisor。** `OutcomeEntrySupervisor` 集中 account-safe、existing exposure、durable ambiguous-submit adoption/fence、reduce-only、方向與 fast-rebook cooldown 等 entry 前置 barrier。任何不確定 submission 仍先採交易所 truth，不允許下一輪重複 BUY。
4. **Research supervisor。** `OutcomeResearchSupervisor` 統一 regime、confidence、active challenger 與 trend-continuation capture。這條路徑只能回傳／寫入觀測，沒有 gateway、private key 或 mutation controller authority，其結果不得改變當輪 production action。
5. **Immutable tick snapshot / thin orchestration。** account recovery 與 fill sync 後建立 frozen `OutcomeRuntimeTickSnapshot`，把同一輪的 market、authoritative recovery report、active findings、entry signal、reduce-only 與 monotonic observation time交給各 supervisor；避免各分支重抓、重解讀會變動的 process-local facts。live facade 只負責依既定優先序組合 supervisor 與既有 atomic execution primitives。
6. **Per-tick read model。** runtime 內已無直接 `sqlite3.connect`。`OutcomeRuntimeJournalView` 以 SQLite `mode=ro` 統一 continuation token、entry policy、latest entry、holding age 與 exact official-fill provenance；同一 tick 的相同查詢 memoize，mutation 不經此層。精確 immutable fill provenance 可跨 tick 快取；缺失／歧義結果不永久快取，後續 official evidence 出現時仍可恢復。每輪 `_begin_tick()` 會清除 decision-local journal 與 account cache，任何 exchange mutation 後的 account truth 仍依原規則 invalidate/re-read。

此重構刻意沒有把巨型檔案中的每個 atomic helper 一次搬家：exchange cancel-confirm、fresh-depth、journal intent 與 reconciliation 已有大量安全測試，機械搬移會增加不必要的 production 風險。完成標準是「決策 ownership、順序、snapshot 與 DB read ownership 已切開」，不是追求任意行數。新增 supervisor ordering/read-only/memoization tests，連同完整 Python regression **312 passed**；official sidecar TypeScript build 與 `git diff --check` 亦需在本次 commit 前通過。後續新增 holding、entry 或 research policy 必須放入相應 supervisor／domain module，不得再把跨領域 control flow塞回 `_tick_live_strategy`。

**F2 — execution-lane latency repair（P0–P2 完成；2026-09-05）：** 調查確認 2–4 分鐘不是 WS polling interval，而是主線同步 read/retry、舊 market settlement SDK 與 SQLite 大表 query 疊加。修正遵循「交易／風控不等歷史會計」原則，沒有新增 `.env` 參數或放寬任何交易權限。

1. **P0 — DB query path（完成）。** `TradeJournalDB` 現建立 `idx_binance_oi_symbol_backfilled_local_time(symbol, backfilled, local_received_at_ms DESC)`、`idx_strategy_events_type_id(event_type, id DESC)` 與 `idx_order_events_type_id(event_type, id DESC)`。前者精確覆蓋 S0 的 `BTCUSDT + backfilled=0 + local_received_at_ms ORDER BY DESC LIMIT 250`，不再使用原本只有 exchange time 的 index 後建 temp B-tree；實際 `outcome_shadow.db` 已驗證 query plan 直接使用新 OI index。現行 live writer 佔用 SQLite 時，兩個歷史 event index 的立即建立以 5 秒 busy timeout 安全拒絕；它們已納入啟動 schema migration，**重啟 bot 時會在新 process 開始寫入前建立**，不得透過強制鎖庫中斷 live writer。
2. **P1 — settlement/PnL 拆線（完成）。** 新增 `OutcomeSettlementWorker`，使用獨立 read-only `OutcomeClient`。official SDK `fetchSettledOutcome`、settlement payout/zero-balance readback、`reconcile_settlement()` 與 FIFO `reconcile_sells()` 全移出主交易 loop，固定 30 秒 scheduler；worker 不 import gateway、沒有 submit/cancel API，無法成為 wallet writer。launcher 主線只交付 retiring/settling market id；worker 仍會從 canonical unresolved lots 補回 restart 前 market。故一筆 `pending_winning_payout_evidence`（例如舊 #1201）可繼續完整追蹤，但不可延遲 current-market entry、cancel/rebook 或 protective sell。
3. **P2 — bounded execution reads + evidence（完成）。** main execution `OutcomeClient` 的 read-only HTTP 為 3 秒 timeout、`info_max_retries=1`；429/5xx/transport error 立刻使當輪 read fail-closed，不能再在 order lane sleep 1+2 秒並重試。WebSocket handshake 維持獨立 10 秒 timeout，不能因縮短 HTTP budget 而提高既有 reconnect false-positive。此規則只適用 read-only `/info`，不把 place/cancel 改成盲目 retry；交易 mutation 的未知回覆仍必須由下一次帳戶 truth reconciliation 決定。每當 live loop 超過 3 秒，journal 才寫一筆 `OUTCOME_LOOP_TIMING`（market/period、total ms、market-discovery/pricing/live-runtime 分段 ms、1.5s target），並在 terminal 警告；正常快迴圈不新增 telemetry，避免重演 raw journal 膨脹。

**F2 verification。** OI query-plan regression 確認無 `TEMP B-TREE`；settlement worker fixture 確認 FIFO 與 official settlement 使用 worker-owned account reads，已 recorded market 不再重輪詢；execution client fixture 確認 502 在 single-attempt mode 不進入 sleep/retry。`py_compile` 通過，完整 Python regression **493 passed**、`git diff --check` 通過。此修正降低 execution-lane 阻塞，並不聲稱外部 API 必定低於 1.5 秒；每次 read failure 仍正確 fail-closed。重啟後以 `OUTCOME_LOOP_TIMING` 的 `stage_ms` 作為唯一主網 latency 證據，再決定是否需要進一步的 parallel public book reads。

**F3 — execution-lane latency optimization（P3 完成；2026-09-06）：** F2 部署後的主網 telemetry 顯示 settlement 已確實拆線（其 log 與交易 loop 可交錯），但仍有 `pricing_reads≈1.7–2.0s`、`live_runtime≈3.3–3.9s` 的 serial 成本。P3 只消除不必要的 read；下單、撤單、protective exit 的交易所真相與 fresh REST L2 仍維持原有 fail-closed 規則，沒有降低任何風控門檻或新增 `.env` 參數。

1. **P3-1 — young resting BUY fast path（完成）。** audited resting entry 尚未到既定 300 秒 requote interval 時，runtime 先由 lifecycle timestamp 判定 `entry_requote_interval_not_elapsed` 並立即保留 order，不再為不可能重掛的 order 額外讀 fresh REST L2。到期、明確 signal contradiction、side change 或可重掛時，仍照原流程取得 fresh book 後才可 cancel/requote；沒有以舊 WS quote 替代 mutation 前確認。
2. **P3-2 — public pricing 優先 healthy WS（完成）。** launcher 先啟動/reconfigure public WS recorder；只有 market-specific stream health 已 ready/resynced、YES/NO BBO 與 BTC mark 都在 15 秒 freshness window 內時，才使用 cache 做公開 pricing/display/S0 gate。新 market、reconnect、任何一側缺值或 stale 都強制 REST fallback；`resync_required` 只能由同輪雙側 REST L2 snapshot 清除，不能由舊 WS cache 自行解除。WS `allMids` 亦持續更新 BTC mark。真正 submit/cancel/rebook 仍各自由 execution runtime 讀 REST book。
3. **P3-3 — per-invocation account read cache（完成）。** `OutcomeAccountReadCache` 在每個 `tick`/`tick_market`/`tick_p3_calibration`/`tick_live_strategy` 開始清空，將相同 wallet 的 spot state、open orders、user fills、fees 在該 decision 內 memoize，避免 recovery、lifecycle、ledger 重複同步 REST read。所有 gateway mutation（BUY/SELL submit、owned cancel、exit replace、emergency IOC）後都立刻 invalidate，故 cancel-confirm、partial-fill/race、inventory/fill readback 永不讀 pre-mutation cache；跨 tick 不保存任何帳戶真相。
4. **P3-4 — attribution telemetry（完成）。** `OUTCOME_LOOP_TIMING` 保留 >3 秒才記錄的低噪音設計，現在將 `entry_gate` 與 `live_runtime` 分開；後者可直接對照 pricing/public-data、account reconciliation、lifecycle mutation 的剩餘時間。下一次 live restart 後，應觀察連續 `OUTCOME_LOOP_TIMING`：healthy WS 時 `pricing_reads` 應接近 cache lookup 而非 1–2 秒 REST；young `buy_resting` 時不應再增加 book read；若 `live_runtime` 仍常 >3 秒，才以其中實際 account endpoint 做下一輪針對性調查，**不得**以猜測降低 reconciliation 或 fresh-book 保護。

**F3 verification。** 新增 young resting entry 不讀 book、WS `allMids` BTC cache、帳戶 cache tick/invalidate、以及「預先快取 open orders 後 cancel confirmation 必須看見新 account truth」regression。P3 定向 lifecycle/runtime/WS/cache tests **57 passed**；完整 Python regression **497 passed**，`py_compile` 通過。這是程式 contract 驗證，不宣稱外部 Hyperliquid API latency 已達成某一毫秒數；主網 telemetry 仍是 P3 實際效果的唯一證據。

**F4/P5 — persistent official SDK、thesis-aware exits、Tier-B execution quality 與 journal retention（完成 2026-09-06；重啟後生效）：** 此項將原 P4 latency attribution 收斂為 execution runtime 的必要改造，而不是額外研究階段。

1. **常駐 official SDK sidecar。** Python `OutcomeSdkSidecarClient` 現以單一 JSON-lines Node child、唯一 request id、response-id match、transport timeout 與健康失敗重建服務所有 `fetch_order_book`、submit、cancel、account/settlement SDK requests；TypeScript child 進一步 cache 同一份 `createHIP4Adapter().initialize()` 與 wallet auth。它沒有換 SDK、沒有改 ALO/cancel/reconciliation 語義；child 死亡、id 不符或 timeout 一律丟棄 child 並 fail-closed。每次回應保存 Python round-trip、SDK handler、PID 與 command timing；gateway 將 timing 附在 submit audit，replacement order 另保存 cancel/book/submit timing。
2. **Replacement/fill audit。** 每個 cancel-confirm-rebook 成功後都寫入 canonical `ORDER_SUBMIT`（新 order id），包含 old id、replacement price/count、trigger-time BBO、loss threshold、當前 signal/reversal 狀態及各 execution timing。因此 `userFills` 的後續 official fill 可直接 join replacement order；不再只靠 lifecycle event 反推。
3. **Thesis-aware loss-band。** 2h、fee-inclusive -5% threshold、fresh BBO 與 `OutcomeReversalClassifier` 的三次連續 opposite signal 必須同時成立才可由 planner 選 `loss_band`。當 fresh same-side context 回復或任一 evidence 缺失，planner 只會重新計算現行 +2% passive target；不會以單次 midpoint 噪音降價。S3 IOC 的獨立更嚴格條件保持不變。
4. **Tier-B execution quality。** Tier-B 在 decision-time 先檢查 BBO spread、前三檔 bid depth 與無人為調參的 bounded policy；bootstrap 使用 125 bps spread、1.25× shares depth、25 bps decision→submit drift cap。累積至少 20 個 compact Tier-B submit audit 後才可用 p10/p90 歷史資料收緊（永不放寬）上述 ceiling/floor。`OutcomeMakerStateMachine` 對送出前重新讀取的 bid 再檢查 drift，超標直接 `flat`、不提交 ALO；每筆 accepted Tier-B audit 保存 policy source、sample count、spread、depth、decision/submit bid 與 drift。
5. **DB retention。** WS `allMids` 不再複製全 Hyperliquid mids map，只保存 BTC + active YES/NO coin，並標記 `recording_scope=btc_and_active_outcome_only_v2`。`scripts/outcome_journal_maintenance.py --report` 可在 live 時只讀；`--prune-legacy-all-mids --vacuum` 僅刪除未標記的新舊全市場 raw map，且必須先停止**全部** journal writers、取得 exclusive lock 與保有約 1.25× DB 的可用空間。不可手動刪除 `-wal/-shm`。本次 historical prune 已成功移除 legacy full-map rows；驗證時 `legacy_all_mids_present=false`、DB 內已有 **2,925,985,792 bytes** free pages 可回收，且 journal 可正常唯讀開啟。受 30 秒工作器限制，後續 checkpoint/VACUUM 未能完成；截至 2026-09-06 主 DB 為 **4,729,360,384 bytes**、WAL 為 **3,880,356,112 bytes**，且已確認無 process 持有檔案。必須在明確停機維運窗口、從本機終端完成 `--checkpoint` 後的 `--vacuum` 才能實體釋出空間；不能在 live runtime 或受中斷限制的工作器進行。

**F4/P5 verification。** official sidecar build 通過；persistent child reuse、response identity、exit replacement audit、thesis-aware loss-band、Tier-B thin/wide/depth/drift gate、WS payload scope、既有 runtime/lifecycle regression 共 **72 passed**。maintenance `--report` 在現行 journal 成功讀到 historical allMids 尚存在；它是 storage 維運 evidence，不是 live 策略信號。

**F4/P5 latency-attribution repair（2026-09-11）。** `OUTCOME_RUNTIME_TIMING` 不再寫 sidecar process 的跨輪 `last_sdk_request`；那個欄位可能屬於先前 tick，不能用來歸因本輪 2–5 秒延遲。每個 runtime tick 現在先 reset gateway timing scope，event 只保存此 tick 真正發出的 `sdk_requests`（command、Python round-trip、sidecar handler/PID timing）；`OutcomeAccountReadCache` 同時保存每個 account endpoint 的 calls、cache hits、實際 network calls、總/最大耗時，作為 `account_read_timing`。因此可區分 recovery/open-orders/fills/fees 的 endpoint cost、fresh-book、submit/cancel 與 journal cost，而不讀取或記錄帳戶內容。此觀測修復不增加 endpoint、下單、撤單或重掛，亦不降低任何 fail-closed gate；下一次正常重啟後才開始產生新 schema 的 timing events。連同 X3 repair 的完整 Python regression 為 **285 passed**。

**F6 — managed-holding execution-lane deduplication（完成 2026-09-06；重啟後生效）。** F4/P5 後的主網 timing 發現已有 managed ALO SELL 時，即使 planner 每輪都回 `requote_hysteresis`／KEEP，runtime 仍串行進行 recovery、holding-path REST L2、S3 fee + L2，以及 exit-requote REST L2，使 `live_runtime` 常為約 4–7 秒。這是實際 decision cadence 變慢，不是單純 terminal log 節流。以下只移除不可能改變當輪交易權限的重複讀取；任何交易 mutation 的 fresh-account/fresh-REST contract 不變。

1. **同 tick observation book 共用。** active holding 的 holding-path、S3 preflight 與 passive exit planner 共用同一張本 invocation 取得的 REST L2；不跨 tick 保存。若 cancel/rebook 或 S3 IOC 已被授權，controller 必須在 official cancel confirmation 後自行再抓一張新的 REST L2 才能送單，絕不把共用 snapshot 當作 mutation 依據。
...
### F5 — capacity-aware sizing 與 $20 live canary（程式完成；$20 已啟用）

**現有容量證據與目的。** 2026-08-23 至 2026-09-06、以台北平倉日的 normal strategy FIFO lots（不含操作者刻意持有到期的 `-$10.5261` settlement incident）合計約 **+$25.41 / $1,286.57 = +1.98%** turnover-weighted realized return；14 個有已平倉日的平均約 +$1.82/day。這不是 scale 後的獲利預測。若錯誤假設 fill quality 與 return 線性不變，$50/$100 daily net PnL 分別需約 $2,530/$5,060 daily turnover，對應目前約 9.2 closed lots/day 為約 $275/$550 per entry；現有 9 筆 v2 fill context 的 BUY top-depth 中位數僅 81 shares（最薄 13），而 #1592 的同側 active 5-minute observed trade notional 中位數約 $305。因此不可直接跳至該量級：大型 passive order 未必立即吃掉 book，卻會成為顯眼 queue liquidity，增加 adverse selection、partial-fill 與 exit-capacity 風險；此資料無法證明有人會「攻擊」本 bot，也不能據此宣稱無風險。

| 項目 | 已授權的下一步 contract | 不做的事 |
|---|---|---|
| **F5-0 — $20 canary** | 在所有 F5 safety gates ready 後，單一 inventory/order、每筆 entry notional 與同 market exposure 同步提高至 **$20**。它是 2x execution canary，不是 $50/day PnL 承諾；按歷史線性假設只約對應 +$3.6/day。 | 不在同一部署提高 direction gate、target、S3 authority 或 $20 以上 size。 |
...
| **F5-3 — all-tier execution quality** | 將原先命名為 Tier-B 的 depth/spread/drift gate 擴展到 Tier-A 與 Tier-B；book capacity 由多次 L2 observation 與去重 WS trades 計算，避免單一可撤銷大單主導 sizing。拒絕 reason 一律使用通用 `entry_*` 命名（例如 `entry_spread_exceeds_calibrated_ceiling`），不得暗示只有 Tier-B 受保護。按 size bucket、spread/depth bucket、order age、partial fill、5/10/30s markout 與 realized result 做 audit。 | 不把目前 9 筆 v2 fill context 或平均深度當成可證明 $250/$550 可安全交易的樣本。 |
...
**F5 activation（2026-09-07）。** 操作者已明確授權啟動 $20 execution canary；本機 `.env` 與版本化 `.env.example` 的 `OUTCOME_MAX_ENTRY_NOTIONAL_USDC`、`OUTCOME_MAX_OUTCOME_EXPOSURE_USDC` 已同步為 `20`，`OUTCOME_MAX_OPEN_ORDERS=1` 不變。此為唯一 live size 調整；Tier-A/B gate、50–55¢ no-trade band、ALO-only 正常出場、S2/S3、dynamic capacity sizing、portfolio guard 與 $10 venue opening minimum 全部保持原狀。要讓設定生效，必須重啟 launcher；正在運作的 Python process 不會重新讀 `.env`。不授權 `$20` 以上 exposure、campaign/splitting 或放寬 loss/taker 權限。

`OutcomePortfolioGuard` 同樣只在此 $20 lane 生效，從 immutable official BUY fills 與 canonical FIFO lots 讀回。自 2026-09-09 起 primary budget 對齊 Outcome **14:00 Taipei 至隔日 14:00 Taipei** 的 market session，而不是日曆日：gross limit 是 configured entry cap 的 10×，session realized-loss limit 是 configured entry cap 的 -40%（$20 時即 $200／-$8）。settlement/account evidence 若在 rollover 後五分鐘內才抵達，仍歸屬剛結束的市場，避免舊市場結算延遲污染新市場的初始 budget。另有帳戶級 rolling-24h realized-loss backstop，為 entry cap 的 -200%（$20 時 -$40），所以一次完整 tail loss 不會封鎖新 24h market，但兩次接近完整尾端損失仍會停止新 entry。兩個連續 confirmed loss 的 30 分鐘 pause、及同一 outcome 已有三個 confirmed losses 時直到 rollover 的 stop 保持不變。所有 session／rolling 實際數字都寫入 admission audit；journal 不可讀即 fail-closed；這不使用 UI PnL 或未結算 mark。S3 的既有 full-inventory depth/cap revalidation 沒有被放寬，$20 測試只會把每次實際 S3 depth/cap rejection 或 submit 保存為 audit，並非宣稱大額 exit 已被市場證實。
...
**F5-7 spread-quality evidence（完成 2026-09-10；read-only，未放寬 live ceiling）。** `outcome_execution_quality_report` 的每筆 F5 submit audit 另保存 entry time-left、Market Regime state/reason、top-3 depth 與 immutable fill-id 對應的 P3 5/10/30 秒 fee-adjusted markout；缺 horizon 明列為 missing，不以零或 UI PnL 填補。`python -m bot.outcome_spread_quality_report --db logs/outcome_shadow.db --period 1d` 依 `0–50/50–100/100–125/125–175/175–250/>250 bps` 分桶輸出實際 submit/fill/realized PnL，並單獨輸出因 `entry_spread_exceeds_calibrated_ceiling` 被拒的候選。拒絕候選僅每 `market × coin × 5 分鐘` 儲存一筆，且只由既有 accepted P2 snapshot 在約 5/15/30 分鐘後補記 public BBO path；它不發額外 REST、不創建 order intent、不改信號、size、ALO、spread cap 或任何 live action。候選 path 是未成交的市場品質標籤，絕不可宣稱為反事實 fill 或 PnL。現行 125 bps ceiling 保持不變；只有累積完整、按 top-3 depth／5m 真實成交流／price／time-left／Tier／regime 分桶且費後不惡化的 evidence，才可提出單一較寬 bucket 的獨立 canary。

### E1–E6 — Milestone A 資金效率研究層（程式完成；shadow/read-only，2026-09-11）

**授權邊界。** Milestone A 的六項工作已完成可執行程式與 deterministic tests，但全部停留在研究／shadow 層。它沒有修改現行 Tier-A/B/C admission、125 bps spread ceiling、$20 capacity sizing、ALO quote、profit target、S2/S3/fast-failure、portfolio guard、market selection 或 wallet mutation。新 shadow class 沒有 gateway/controller reference；所有輸出均帶 `read_only`／`execution_submitted=false` 或 `live_authority=false`。報表不得自行升格為 live policy，任何 entry、marketable profit exit 或多市場資金配置仍須新的操作者明確授權。

| 項目 | 已完成的 Milestone A contract | 尚待的真實市場證據／後續授權 |
|---|---|---|
| **E1 — capital-efficiency funnel** | `python -m bot.outcome_capital_efficiency_report --db logs/outcome_shadow.db --period 1d` 以 admission decision 間隔區分 capital committed、BUY submitted/resting、無方向 edge、execution-quality／pricing rejection、safety block 與其他 idle；每段最多歸因 30 秒，避免把 process downtime 偽造成策略等待。另由 canonical closed FIFO lifecycle 計算 submit→fill、holding time、capital-hours、realized net 與每 capital-hour net。 | admission tick 是相關觀測、不是獨立交易；缺少 process evidence 的長空窗不回填。它用來找 bottleneck，不自動放寬 gate。 |
| **E2 — executable fair-value model（calendar extension 2026-09-12）** | 先以 `python scripts/build_outcome_oi_features.py --db logs/outcome_shadow.db` 建立既有 leak-free X3 rows，再執行 `python -m bot.outcome_fair_value_model --db logs/outcome_shadow.db --horizon-sec 300 --artifact logs/outcome_fair_value_model.json`。模型以 YES/NO BBO、spread、price distance、time-left、5m/15m/1h Binance mark、5m OI activity，及 B1 相同的 Taipei market-session weekday/weekend calendar/interactions 預測五分鐘後 executable bid；每 market/side 每 horizon 最多一列，並按完整 daily market 做 chronological walk-forward。artifact 永遠標記 `live_authority=false`。 | calendar 只可由 session timestamp 導出，不能輸入 raw date；weekend/weekday pattern 必須在未見 daily market 比較並分桶報告。至少 5 個獨立 daily markets、100 個非重疊 rows 與可用 market walk-forward 才移除 research blockers；仍需比較 OOS RMSE、calibration 與 baseline。建立 derived X3 table／artifact 應在停止 live writer後或一致性 DB copy 上進行，不能讓重建工作阻塞交易 loop。 |
| **E3 — confidence-aware entry shadow** | launcher 重啟後，每次 S0 decision 用凍結 E2 artifact 對兩側輸出 predicted executable bid、gross edge、單側 95% lower edge 與 confidence；只在狀態變化或 30 秒 cadence 寫 `OUTCOME_CONFIDENCE_ENTRY_SHADOW`。artifact 缺失／特徵不完整時明確 `unavailable`，不猜測也不擋現行 entry。 | 必須以後續獨立 market 的 realized executable path 與 actual lifecycle 分桶驗證 calibration；目前不得用 confidence 增減 size、選邊或拒絕 entry。新 artifact 只在 process 建立時載入，更新後需正常重啟才開始新的 shadow scoring。 |
| **E4 — queue-aware pricing shadow** | 現行 fresh entry REST L2 取得後，在記憶體比較 `SKIP`／`JOIN_BEST_BID`／`IMPROVE_ONE_TICK`，保存 spread、observed tick、best-bid queue-ahead proxy、requested shares、fill-share proxy 與 fair-value lower edge至 `OUTCOME_QUEUE_PRICING_SHADOW`；production bid 明確原樣保存，且不增加 REST call。 | queue-ahead 是 L2 proxy，不知道撤單、優先序或成交歸屬；必須和 official fill probability、submit→fill、短期 markout、cancel age 比較後才能提出定價 canary。 |
| **E5 — +1%/+2% marketable profit exit counterfactual** | 新 holding-path observation 會用同輪 fresh REST 的完整 bid levels 對**全部 current inventory**做 depth walk，只有深度足夠才保存 marketable VWAP；再套用 entry-time persisted taker fee。`python -m bot.outcome_marketable_profit_exit_report --db logs/outcome_shadow.db --period 1d` 比較首次 fee-after +1%/+2% 可執行時點的 hypothetical net PnL 與 canonical actual realized net。 | 舊 top-BBO rows、taker fee 不明或完整倉位深度不足一律排除。displayed depth 仍不是 IOC fill 保證，報表永久 `ready_for_live=false`；沒有因此新增一般 take-profit taker 權限。這項新 evidence 需重啟後才開始累積。 |
| **E6 — multi-market portfolio allocator** | `OutcomePortfolioAllocator` 是純函式 shadow allocator，以 `lower edge / expected capital-time` 排序，受 global、per-market、correlation-group、safe-capacity、最大 active markets 與官方 $10 minimum 約束；不足最低額不切碎補單，剩餘 budget 不製造後續 order。 | 現行 launcher 仍只交易單一 active BTC daily market。未來多市場 live 必須先完成中央 wallet/account truth、跨市場 reservation、correlation exposure、partial-fill/restart reconciliation 與 portfolio emergency controls；本模組不能匯入 wallet key 或直接 submit。 |

**統一驗收命令。** `python -m bot.outcome_efficiency_milestone_report --db logs/outcome_shadow.db --period 1d` 一次輸出 E1、E2、E3/E4 event count、E5 與 E6 implementation status。`milestone_a_complete=true` 只代表上述 research implementation 已完成，**不代表資料已足夠、策略有效或 live 已授權**。若 `outcome_oi_feature_rows` 尚未建置，E2 正確顯示 blockers；若 launcher 尚未重啟或 frozen artifact 不存在，E3/E4 count 可為零；若尚無新 full-depth/taker-fee holding observation，E5 也會正確顯示零 eligible lifecycle。這些是證據不足，不是用零值替代結果。

**研究 checkpoint — 2026-09-11 Asia/Taipei（可重跑、不可外推）。** 本段只記錄當日停止 journal writer 後，依序完成 X3 incremental build、X4 market walk-forward、X4 intraday report、E2 fair-value training 和 Milestone-A report 時的 DB 截面。未來重跑必須新增一個帶日期的 checkpoint；不得以本段數字覆蓋、平均或外推另一日期的資料。所有結論均為 research-only，沒有改變任何 live gate、notional、ALO/taker 權限或 `OUTCOME_TREND_CONTINUATION_ENABLED=0`。

1. **資料與 OI 結論。** X3 build 當次補建 58,697 rows，work rows 全部有 live as-of OI join，並產出 54,092 個 60m executable labels。完整 X4 market-instance walk-forward 使用 13 個 daily markets、73,036 eligible rows、69,594 OOS rows：Outcome-book baseline 的 RMSE/MAE 為 `0.0297191/0.0184524`，加入 OI 的 extended model 為 `0.0298488/0.0185991`，RMSE/MAE improvement 分別為 `-0.0001297/-0.0001468`。intraday 5m provisional result亦為負（baseline RMSE/MAE `0.0346532/0.0188256`，OI extended `0.0350112/0.0193424`）；15m 尚無足夠 purged OOS rows。結論是：截至此日期，OI 沒有可證明的 incremental executable-prediction edge，必須維持研究資料，不得作為放寬 entry 或選邊依據。
2. **Fair-value shadow。** `outcome_fair_value_model` 使用 5,784 non-overlapping rows、13 markets；11 個 chronological market folds、4,958 OOS rows的 model RMSE 為 `0.0331489`，mid-persistence baseline 為 `0.0337480`，其中 8/11 folds 較 baseline 改善。這支持保留 frozen artifact 作 E3 shadow observation，但改善幅度小、模型含 OI activity 特徵且尚未驗證 confidence calibration／actual fills，artifact 永遠維持 `live_authority=false`，不得直接進入 entry policy。
3. **資金效率不是 execution-quality bottleneck。** E1 的 16,346 admission observations 以每段最多 30 秒的保守歸因計，`no_directional_edge=62.327%`、`capital_committed=17.202%`、`safety_blocked=10.655%`、`pricing_rejected=9.134%`、`execution_quality_rejected=0.578%`。這些是 correlated observed ticks、不是「可任意放寬並下單」的市場時間比例；它們支持優先研究 regime/entry policy 與 holding lifecycle，而非以放寬 spread 或忽略風控來追求周轉。當時 92 個 F5 BUY submits 中 32 個 official fills（34.78%）、29 個 completed lifecycles；holding median `645.094s`、mean `3311.349s`，顯示少數 long-tail holds 佔用大量 capital-hours。
4. **不可採用純價格 stop。** 29 個 lifecycle-level holding paths 中，11 個曾 breach `-5%`，其中 10 個後來回本、9 個到達 target；9 個 breach `-10%`，其中 8 個回本、7 個到達 target；2 個 breach `-20%`，其中 1 個回本且到達 target。此資料反對把 fast-failure 簡化成「一到 -5% 或 -10% 就賣」；現行 live fast-failure 必須同時要求 official fill age、persistent opposite thesis、fresh full depth、fee-after cap 與 one-shot token 的 thesis-aware contract 保持不變。下一個 exit 研究應比較這些**聯合條件當時是否成立**與最終 recovery，而不是只按價格閾值調整。
5. **Trend-continuation 尚不可 promotion。** C1–C3 報表有 104 個完整 2h BBO episodes、但只跨 4 個 independent daily markets（C5 最低要求為 5）。候選中 `+1%/+2%/+5%` BBO path hit 分別為 83.65%/76.92%/55.77%，但 `-5%/-10%` hit 亦高達 56.73%/40.38%，且 53/104 同時觸及 `+1%` 與 `-5%`。因此它不是安全的「strict S0 少開單」替代品；截至此日期，不啟用 continuation canary。待第 5 個獨立 daily market 完整收集後，仍需確認 weekday/weekend、price/time-left/spread bucket 的 downside path 未相對 strict baseline 惡化，才可重新提案。
6. **一般 marketable profit exit 尚無結論。** E5 只有 5 個有完整 position depth 與 entry-time taker fee 的 lifecycle，沒有一個曾滿足 fee-after `+1%` 或 `+2%` 的全倉、depth-sufficient marketable threshold。不得因既有 maker exits 的正 PnL 推論 taker profit exit 會更好；繼續收集完整 depth/taker-fee observations。

### B1–B6 — 主動決策 challenger（B1–B5 程式完成；B6 禁止 live，2026-09-11）

**目標與第一性原則。** Milestone B 不是把既有 gate 全部拿掉，也不是把 GPT／LLM 接到 private key 後直接選邊。它把「大部分時間等待、錯過 0.59→0.92、少數倉位一路持有至 tail loss」拆成可驗證的 supervised targets，再由 deterministic、fee/depth/capital-time aware optimizer 比較 WAIT、被動報價、受限 marketable entry 與持倉 exit。模型只能提供預測分布；account truth、freshness、ownership、capacity、portfolio guard、price cap 與 official SDK mutation boundary 永遠由確定性代碼掌握。

| 階段 | 完整計劃與現況 | B6 前不得省略的證據／限制 |
|---|---|---|
| **B1 — decision dataset（完成；calendar extension 2026-09-12）** | `bot/outcome_active_dataset.py` 由 X3 `outcome_oi_feature_rows` 以每 market/side 60 秒 event-time cadence 建立 compact rows。特徵包含 side mid/spread、距 50%、time-left、side-signed 5m/15m/60m Binance mark、5m OI activity，以及以**台北 14:00 market session**為準的 `is_weekend`、weekday cyclic sin/cos，並加入 weekend×spread、weekend×5m-momentum 交互項；targets 包含 5/15/30/60m executable future bid、離散 horizon MFE/MAE、+1/+2/+5% hit/time、-5/-10/-20% breach/time 與 breach 後 recovery。`dataset_report` 和 B5 unseen report 均按 weekday/weekend 輸出樣本、action 與 fee-adjusted future-bid path。 | 原始日期（如 `2026-09-12`）絕不作特徵，避免模型記住單日 market。14:00 前的午夜時間仍屬前一 market session；週末樣本不足時，它只是 regularized read-only feature，絕不自動成為週末停機／放寬 rule。X3 的 path label 只在既有 horizon 觀測，不是連續 tick path；future **bid** 才是 exit evidence。不得把 maker touch 當 fill、把 settlement 勝負當任意時間可成交價格，或把同一 daily market 的相鄰 rows 隨機切入 train/test。 |
| **B2 — multi-target model（calendar extension 2026-09-12）** | `python -m bot.outcome_active_model --db logs/outcome_shadow.db --period 1d --sample-interval-sec 60 --artifact logs/outcome_active_model.json` 以可審計 ridge models 同時估 5/15/30/60m future bid、target/loss/recovery probabilities，並保存 residual q10/q50/q90。模型直接使用 B1 calendar features；validation 按完整 daily market chronological walk-forward；maker fill probability 只由 official BUY submit/fill order id 估計並按 known spread bucket 分開，legacy unknown spread 不套到有 live spread 的決策。artifact 固定 `live_authority=false`。 | 每個 continuous target 必須與同 OOS rows的 persistence baseline（future bid=current bid）比較；model 未勝 baseline 就是明確 blocker，而不是以 sample 大掩蓋。weekday/weekend 係數本身也不是 evidence：必須另按 unseen market/day-type 報告 action/path，且每類樣本足夠才可提出 live policy。probability targets 仍須校準、tail/recovery 仍需跨 weekday/weekend/time-left 驗證。更新 artifact 應在 writer 停止或一致性 DB copy 上完成，runtime 只在重啟時載入 frozen artifact。 |
| **B3 — action optimizer（完成 shadow-only）** | `OutcomeActiveActionOptimizer` 是無 client/gateway/journal 的純函式；候選動作為 `WAIT`、`JOIN_BEST_BID`、`IMPROVE_ONE_TICK`、`BOUNDED_MARKETABLE_BUY`，持倉候選為 `HOLD_OR_PASSIVE_EXIT`、`MARKETABLE_PROFIT_EXIT`、`BOUNDED_RISK_EXIT`。maker utility 使用 predicted mean、實際 spread-bucket fill probability、tail penalty 與 capital-time cost；marketable entry 必須以 q10 fee-after edge、target probability 與 tail cap 通過較嚴門檻。即使 WAIT 也保留最佳 rejected side，供未來 path 審計。 | 目前數值都是研究假設，`execution_submitted=false`、`live_authority=false`。B3 不知道真實 L3 queue；maker 候選只能是 conditional opportunity。marketable profit/risk action 不等於 execution authority，亦不取代 S3/fast-failure。 |
| **B4 — purged market replay（完成）** | `python -m bot.outcome_active_replay --db logs/outcome_shadow.db --period 1d` 每個 test daily market 只使用更早 markets 訓練。只有 `BOUNDED_MARKETABLE_BUY` 可用當下 ask、future bid 與 fee 計 executable counterfactual return；JOIN/IMPROVE 只報「若有 maker fill」的 conditional path，不計 fill/PnL。actual canonical realized PnL 只作旁列 context，不能冒充同 allocation 的 baseline。 | replay 沒有 L3 priority/cancel race，也沒有為 maker 候選創造假 fill。B4 的好結果不能單獨 promotion；必須由 B5 的 frozen, unseen, live-as-observed shadow evidence 驗證。 |
...
**實作驗收與 live 邊界。** B1–B5 新模組與 runtime integration 的 focused regression 為 **53 passed**；完整 Python suite **292 passed**、official SDK sidecar TypeScript build 與 `git diff --check` 通過。所有 shadow payload 都明列 `execution_submitted=false/live_authority=false`。本次沒有更改 `.env`、現行 admission、$20 sizing、ALO/IOC 權限、spread ceiling、portfolio guard 或任何交易 mutation。回答操作者的流程問題：**是，必須先重啟收集 B5/B6 所需 unseen 資料，完成分析並再次明確授權後，才可以另行實作 B6 live production；收集完成不等於自動啟用。**

### B5 exit evidence extensions — profit-lock 與 crash-circuit 研究（程式完成；read-only，2026-09-12）

**問題與範圍。** 2026-09-11/12 的一筆 YES lifecycle 曾在約 `93.1¢` official fill 後，於 B5 warm-up 完成後出現多次 fee-after、full-inventory depth-sufficient `+1.17%` 至 `+1.74%` 的 `MARKETABLE_PROFIT_EXIT` shadow action；隨後價格急跌，既有 fast-failure 沒有取得三方反向確認。這是重要的**個案**，不是 B6 的證據或 crash prediction。以下兩條資料線的目標，是以未來 unseen lifecycle 比較「有利可成交時 profit lock」與「價格、深度、外部訊號共同惡化」；它們絕不改變現行 ALO exit、S3、fast-failure 或 IOC 權限。

| 研究線 | 實作 contract | 明確非功能／升格門檻 |
|---|---|---|
...
**Holding-risk 三分流 shadow（完成程式與 focused tests，2026-09-18）。** 為避免把「單邊持續惡化」與「交錯、depth 回補、spread 收斂後的回升」混成同一種 IOC 候選，`OutcomeMarketRiskMonitor` 在既有 90 秒 bounded WS BBO path 額外計算 bid trend efficiency、bid direction flips、depth recovery ratio 與 spread contraction ratio。它只在既有 fresh REST full-depth capture 已經到達 `HARD_CANDIDATE_WITHIN_CAP` 或 `HARD_CANDIDATE_DEPTH_OR_CAP_BLOCKED` 的邊界，寫入 compact `OUTCOME_HOLDING_RISK_DECISION_SHADOW`。每筆同時保留三個 immutable counterfactual：`HARD_IOC_COUNTERFACTUAL`、`WARNING_ONLY_COUNTERFACTUAL`、`HOLD_FOR_RECOVERY_COUNTERFACTUAL`；後者只有在「至少一次方向翻轉、trend efficiency <= 0.60、depth 自最低點回補至少 1.25 倍、spread 收斂至最高值的 0.80 以下」都成立時才標記 eligible。這些數字是**預先宣告的研究定義**，不是已校準 stop 參數。

**Live boundary。** 三分流 event 明列 `read_only=true`、`live_authority=false`、`execution_submitted=false`；它不會取消/改價 protective SELL，也不能否決現有 narrow hard-failure canary、fast-failure 或 S3。`HOLD_FOR_RECOVERY` 要取得任何 veto authority 的先決條件，是 warning lane 先以既有 exit lifecycle／durable intent／ambiguity fence 證明能提供替代保護，並在 independent unseen hard-candidate episodes 的 post-exit 5/15/30 分鐘 executable continuation 中同時量化 false-stop opportunity cost 與避免的 tail loss；達成前，現行 live controller 完全不讀這個 shadow 結果。此 lane 不增加 REST request：只重用既有 holding-path 已取得的 full-depth book，journal 寫入採 best-effort 且僅在 boundary state/shape 改變或既有 30 秒 holding-path cadence 時發生。
...
**B5 feature availability correction（2026-09-12）。** 初次上線檢查發現 B5 的 `15m/60m` mark feature 曾錯誤依賴 `trend_continuation` payload：當 spot 未滿 15 分鐘 persistence 而 continuation 正確拒絕時，該 payload 未填長週期 mark，導致 B5 全部輸出 `active_*_features_incomplete`，即使 OI/mark history 本身足夠。已改為每個 S0 evaluation 都直接由同一份 fresh local Binance OI/mark history 計算並保存 `mark_15m_bps`／`mark_60m_bps`；continuation 的 spot-persistence、long-horizon alignment 與 canary eligibility 完全不變。這只修復 shadow feature availability，不改 entry/exit 行為或任何 live authority。新值需要 launcher 正常重啟後才會寫入；process 啟動初期真實不足 60 分鐘 history 時，B5 仍應誠實標記 unavailable。
...
**G9 — official-fill visibility fence（完成 2026-09-07）。** 在 #1715 的正式 journal 中，第一張 15-share NO BUY 已有 immutable official user-fill（`70.167¢`），但後續 account balance/open-order read 仍暫時回報 flat；舊 runtime 因只信該次帳戶快照而在約 6 秒後送出第二張 15-share BUY（`67.963¢`），使單 market exposure 暫時達 `$20.72`，超過當時 `$11` cap。這不是 partial fill 或 UI duplicate，而是 fill-to-account visibility lag。現在每個 S0 admission 在 sync official fills 後，都檢查 durable owned entry lifecycle：若已確認 BUY fill、但同一 coin 尚未由 account truth 顯示 inventory 或 protective sell，lifecycle 轉為 `FILL_PENDING_RECONCILIATION`、寫入 `OUTCOME_ENTRY_FILL_VISIBILITY_LAG_BLOCK`，並拒絕任何新 BUY。若 owned BUY 無 cancel/fill terminal evidence 就從 account 消失，亦進入 `RECONCILE_REQUIRED`，不允許以新 submit 覆寫 lifecycle。只有帳戶觀察到 inventory 或 sell 後才寫 `FILL_RECONCILED`，解除這個 fence；fill 僅作 exposure fence，絕不可用來推斷 protective sell size。

**G9 verification。** 新 regression 模擬「official user-fill 先到、balances 與 open orders 延遲仍為 flat」的真實 race：第一張 accepted BUY 之後第二次 S0 tick 必須為 `blocked: official buy fill pending account reconciliation`，gateway 的 BUY submission count 必須維持 1，且 journal 必有 `FILL_PENDING_RECONCILIATION` 與 lag-block audit。再模擬 authoritative account inventory 出現，runtime 必須解除 fence 並走既有 protective ALO sell path，仍不得再送 BUY。此修正是 P0 risk invariant，未驗證前不得開始 `$20` canary；修正本身不改 live caps、也不下新交易所訂單。

**G10 — 移除 Telegram control / notification plane（完成 2026-09-07）。** Outcome launcher 不再 import、建立或 background-poll Telegram controller；`telegram_bot.py`、`telegram_notifier.py`、`alert_watcher.py` 及其 tests 一併移除，`python-telegram-bot` 亦不再是 dependency。本機 `.env`／`.env.example` 和 runtime allowlist 已移除 Telegram token、chat id 與 controller settings。這消除同一 token 被另一個 `getUpdates` poller 終止、或 Telegram thread 影響 live process 的失效面；交易、risk、official SDK、SQLite journal、terminal dashboard 與 process lock 均不依賴 Telegram。現有正在執行的舊 process 必須重啟後才會停止 Telegram polling。

**G11 — lifecycle-level holding-path outcome report（完成 2026-09-07；timing correction 2026-09-09）。** `OUTCOME_HOLDING_PATH_OBSERVATION` v2 現在只在 current account inventory 可由**單一 immutable official BUY fill**精確重建時，寫入 `entry_lifecycle_id=official_buy:<order_id>:<trade_id>`、entry order/trade id、official fill timestamp、side、Tier-A/B、target return 與入場距結算時間。v3 的 `holding_age_sec` 一律從 official `timestamp_ms` 成交時間計算，並寫入 `entry_filled_at_source=official_fill_timestamp_ms`；絕不可從 `ENTRY_PLACED`（order submit）起算，因 ALO resting 時間不屬於持倉時間。既有 v2 lifecycle rows 的 `entry_filled_at` 是本地 journal 收到 fill 的時間，唯讀報表仍以 observation event time 減該 timestamp 重算 age，且明標 `legacy_journal_fill_timestamp`；不可與新 v3 的精確 timing 混作精密短時窗證據。partial/multi-fill、重啟後歷史不足或任何 vwap/quantity 不完全相等的 composite position 保持 unlabelled；絕不以「同一 coin 最新 entry」猜測歸因。新唯讀命令 `python -m bot.outcome_holding_path_outcome_report --db logs/outcome_shadow.db --period 1d` 對每個 lifecycle 報告 MAE/MFE、`-5%/-10%/-20%` breach、兩小時後 breach、breach 後是否回本／觸及 entry-time target、final canonical FIFO close status，並以 Taipei weekday/weekend、Tier、兩小時後同方向 signal re-confirmation、entry time-left bucket 聚合。它不讀 exchange、不提交／取消 order，也不改 S0/S2/S3。舊 v1 `(outcome, coin)` observations 沒有 lifecycle id，會顯式列為 `legacy_or_ambiguous_observations_excluded`，不可回填或用來宣稱 weekend/recovery predictive evidence。首次部署前 DB 的 12,404 筆舊 observations 全屬 legacy；最新 smoke report 已開始輸出 1 個 v2 lifecycle（另仍排除同 12,404 筆 legacy），證明新的 immutable entry binding 已實際寫入，但樣本量尚不足以作任何 weekday/recovery 推論。
...
**C5 promotion gate（尚待市場證據，禁止自行跳過）。** `OUTCOME_TREND_CONTINUATION_ENABLED` 在實際環境保持 0，直到報表有至少 20 個完整 2h candidate episodes、至少 5 個彼此獨立 daily markets、且 weekday/weekend、price/time-left/spread bucket 不顯示相對 strict baseline 明顯惡化的 -5% BBO path；此為研究最低條件，並不自動授權 live。其後仍須操作者明確同意有限 canary，並累積至少 10 個 official filled continuation lifecycles、完整 holding-path / exit / S3 audit 與無 duplicate/unprotected inventory evidence，才可提案移除 temporary env switch、成為正式 code-owned policy。若 evidence 不足、BBO path 遇到較多 -5% breach、或 canary 出現 ownership/capacity/exit failure，立即關回 0；strict S0 不受影響。

**F5 sizing 與 re-entry 的關係。** 同一 daily market 的 verified loss 後，S2-3 對每一張新 entry 都套用 cooldown/reclaim 條件；F5 的 dynamic size 只決定該合格 entry 的安全 shares，不能繞過 S2-3、風控、portfolio circuit breaker 或以小單規避任何上限。accepted resting BUY 的紀錄是 audit，不是 re-entry 次數額度。所有上限是程式內固定 product policy，目的是降低 `.env` 人工輸入與因操作錯誤改變 live risk 的機會。
...
7. **Fast-failure emergency exit（live；2026-09-09）：** 這是處理新倉快速 thesis failure 的唯一早期 taker-like 例外，與兩小時後 S3 分離。必須同時有 (a) immutable official fill age ≥60 秒（ALO submit/resting 時間不得算入）、(b) net executable depth-walk loss 已達 **-10%**、(c) 三個彼此至少 60 秒的 persistent reversal confirmations、(d) fresh REST full depth 可將**完整**庫存以 taker fee 後最多 **-15%** 的價格 cap 賣出、(e) 此 lifecycle 尚未用過任何 emergency IOC。才會 cancel-confirm 現有 managed ALO sell、重新讀 fresh L2 並透過 official SDK 送一次 full-size price-protected FAK/IOC SELL。無 official fill provenance、資料不新、反向不持續、深度不足、價格跳過 cap、取消未確認或 IOC 回應不完整均 fail-closed，保留原本 reduce-only ALO；絕不送裸市價單、絕不刻意以部分倉位下單、絕不因虧損自動反向或加倉。FAK 在提交瞬間若流動性消失仍可能部分成交；此時 official fill／inventory reconciliation 決定剩餘倉位，絕不可把 full-depth precheck 誤報成保證全數成交。它的目的是將確認失效的 full-loss tail 轉為有明確上限的**嘗試**，不是保證能在 -15% 成交。
8. **S3 long-horizon emergency exit：** 若被動 loss-band 已 resting 至少 20 分鐘仍未成交，且 holding ≥2h、三個彼此至少 60 秒的 persistent reversal observations、fresh full depth、fee-inclusive -8% trigger 與 -12% maximum cap 都成立，才可使用同一份 remaining emergency IOC token。若 fast-failure 已成功提交／嘗試，S3 不得再送第二次 IOC；反之 fast-failure 未通過任何 gate 不消耗 token，S3 照常在其獨立條件成熟時評估。兩者皆不是正常 take-profit、一般減倉或每次反向可使用的 taker 權限。
9. **loss 後重進、平倉與重啟後：** official fills／account truth 先完成 FIFO reconciliation。managed verified loss exit 後固定冷卻 15 分鐘；冷卻後每一次 entry 都必須重新通過當下 S0/Tier、fresh book、risk、fee-after target headroom。相同 side 還必須收復前次 official loss exit bid；不同 side 不自動反手，僅在當下完整新方向訊號成立時才可成為候選。official SDK 接受 resting BUY 後會記錄 audit，但不會消耗 daily-market 次數額度；因此未成交 ALO 被取消或新的獨立合格訊號不會被永久拒絕。沒有 loss exit 時，確認 flat 且不存在 managed order即可重新從當下 S0/Tier gate 選邊。重啟 recovery 只接管有 matching durable audit、market/coin、inventory 與 ownership 的 managed order；未知／手動訂單一律不接管。
...
**G13 live authority boundary。** G13 state machine 本身仍不改變 Tier-A/B admission、$20 sizing、H1 fast cancel-only、一般 ALO take-profit、S2 loss-band、re-entry 或 portfolio guard；尤其 `TRANSITION` 不會取消現有 order，`RANGE` 不會阻擋 entry，`TOXIC_FILL` 不會 reprice、cancel protective sell 或送 taker/IOC，所有 G13 state event 均維持 `read_only=true` 與 `execution_submitted=false`。2026-09-09 另經操作者明確授權的 fast-failure IOC 不讀取 G13 state 作為權限，而是採用上方第 7 點既有 reversal evidence、official fill clock 與 fresh full-depth gate；不得將這項 live authority 外推為允許任何 G13 shadow state 下單。
...
**G14 — external-fill origin audit（完成 2026-09-09）。** `userFills` 只能證明交易所已成交，不能自行揭露該 order 是 UI 手動送出或 runtime 送出。因此每個新 `ORDER_FILLED` 在首次 immutable 寫入時附帶 `execution_origin`：只有該 exact venue order id 存在較早的本地 Outcome `ORDER_SUBMIT` 時才標為 `bot_managed`，並保存 matched submit event id；沒有相符 submit，或 fill 缺 venue order id，必須標為 `external_manual_or_unknown`，附上原因。這是保守的來源歸因，不把未知外部 API／另一個 process 的成交誤稱為人手操作。它不改變 FIFO PnL：所有 official fills 仍照交易所 timestamp 入帳。runtime 不會接管未知／手動 order；操作人員若要手動平倉，必須先停止 runtime 並確認／取消 managed protective sell，否則下一輪 account reconciliation 可能重建保護單或進入 fail-closed reconciliation。
...
stateDiagram-v2
    ...
    WAITING --> ACTIVE: 開盤至 daily 剩餘時間 1h
    ACTIVE --> REDUCE_ONLY: 1d/24h/daily 剩餘 3600s；短週期剩餘 300s
...
| :--- | :--- | :--- | :--- |
| **等待發現** | `WAITING` | 監聽 `outcomeMeta`，解析已配置 period 的 BTC market 與 strike。 | 距離開盤 $> 0\text{s}$ 或無活躍市場 |
| **做市活躍** | `ACTIVE` | 現行 1d S0 僅在 OI/mark/spot-strike confirmation 合格時提交 Maker Post-Only (ALO) BUY。 | 1d/24h/daily 剩餘時間 $>3,600\text{s}$；短週期 $>300\text{s}$ |
| **僅允許減倉** | `REDUCE_ONLY` | 禁止新 BUY，帳戶真實 order reconciliation 取消 owned BUY；既有 ALO SELL 不因尾段而取消。 | 1d/24h/daily $0<time\_left\le3,600\text{s}$；短週期 $\le300\text{s}$ |
| **協議結算** | `SETTLING` | 保留 retiring／journal-recovered market，官方 SDK 查驗 settlement；只有 SDK binary settlement 與 official payout／zero-balance evidence 一致時才寫一次 `MARKET_SETTLEMENT`。 | 剩餘時間 $\le 0\text{s}$ |
...
此架構不允許複製多份 repo、替每份 `.env` 放不同 private key，再以不同 `LIVE_PROCESS_LOCK_PATH` 繞過 single-writer。那種部署會讓每個 process 各自通過 capacity、daily turnover、loss breaker、post-loss cooldown/reclaim 與 open-order gate，卻沒有人知道合計曝險；同一時間讀取舊 book 的多個 process 還可能各自宣稱同一份 touch capacity。多台機器若共用 NAT/public IP，也不能假設 API/WS 配額彼此獨立。
...
4. **結算不隨選擇消失。** launcher 對 retiring market，以及 restart 後由 canonical FIFO 未結 lot 找回的 market id，持續透過 official SDK `fetchSettledOutcome` 查驗，30 秒節流；BTC 價格、前端概率或本地推論均不得代替 settlement/payout evidence。只有 winning payout／losing zero balance 與 SDK settlement 一致時才寫 canonical PnL；即使 SDK 已確認，舊 coin 的餘額／掛單仍會由 recovery 繼續約束新 entry。
...
**Testnet venue evidence (2026-09-17).** A disposable testnet GTC BUY on
Outcome `19579` / `#195790` was placed at `0.01 × 1000` and confirmed
resting. A first size reduction to `999` was safely rejected before venue
mutation because it would have violated the $10 minimum notional; the old
order remained visible. A size-only increase to `1001` then succeeded, but
the official SDK ACK reported old OID `60309961112` while the fresh official
open-order snapshot showed a new resting OID `60309987811`. A subsequent
price-only edit `0.01 → 0.01001` also succeeded and changed the visible OID
again to `60310053555`. The order was cancelled and the final official
snapshot showed zero open orders, zero hold and zero positions. Therefore
the testnet venue has confirmed a critical lifecycle fact: **even a successful
size-only modify cannot be treated as preserving the local OID**. Queue
priority cannot be inferred from this test (the book was empty), and no
lost-ACK or partial-fill race was induced. `modifyOrder` remains shadow-only;
any future use must first adopt the replacement from fresh account truth under
the existing durable ambiguity fence. The probe was also corrected to accept
official whole-share strings such as `1001.0` rather than rejecting them as a
local parser error.
...
| `RECOVERY_EXIT_ENABLED` | legacy / shadow | 不會啟用 Outcome live taker／IOC stop；正式 Outcome path 僅 ALO。 |
...
### 2026-09-16 — hard-exit re-entry integration and unified replay (implemented; no canary expansion)

**Confirmed narrow-IOC loss now enters the existing S2-3 gate.** A narrow hard-failure IOC that returns `emergency_exit_flat` now first synchronizes official `userFills`, then calls the existing `OutcomeLossReentryGate`. That gate independently requires the exact bot-owned BUY/SELL lot to reconcile as fee-inclusive negative PnL before writing `OUTCOME_LOSS_EXIT_CONFIRMED`. Only then do its fixed **15-minute** cooldown, same-coin reclaim of the official exit price, and current S0/Tier/fresh-book/fee-after admission apply. Accepted post-loss resting BUYs remain durably audited, but they are not a per-market attempt budget: a cancel or a later independent qualified setup cannot permanently lock the market. This is an integration repair, not a new stop or a new operator setting. Partial/zero IOC fills, ambiguous ACKs, account/journal uncertainty, profitable exits, or an incomplete official lot do not record a loss and do not activate the post-loss guard. It is intentionally not retroactive: historical exits remain forensic evidence, not newly fabricated lifecycle facts.

**One replay instead of more live observers.** `scripts/outcome_exit_decision_replay_report.py --db logs/outcome_shadow.db --period 1d` is a read-only join over existing `ORDER_SUBMIT`/official fills, holding path, Market Risk Monitor, Crash Circuit and Fast-Failure shadow rows. It reports every executed narrow IOC, factual later official re-entry, and the pre-exit 90-second bid-velocity direction flips, top-3 depth recovery/depletion and spread contraction/expansion. `--include-raw-ws` additionally reads raw WS L2 for a post-exit BBO path; it is deliberately offline/possibly slow and must run against a stopped writer or consistency copy. The report emits only `persistent_or_unresolved_deterioration`, `chop_depth_recovery_candidate`, or `insufficient_monitor_coverage`; none is a live authorization.

**Initial factual review is not a rule calibration.** The first two $11 canary exits in market `#3253` were at 64.815¢ after a 72¢ entry and 70.043¢ after an 81.6¢ entry. The first factual re-entry filled 190.6 seconds later at 67.25¢; the second had an unfilled BUY attempt after 389 seconds and first re-filled 3,629.1 seconds later at 73.34¢. The replay labels the first pre-exit window `persistent_or_unresolved_deterioration` and the second `chop_depth_recovery_candidate`; two episodes are evidence to investigate, not evidence to change an IOC threshold. The required next review compares these, known tails and #2639 using a predeclared raw-WS control window, recovery horizon and realised re-entry cost. No additional runtime telemetry, schema, entry rule, stop threshold, or canary size was added for that analysis.
...
After the existing protective-SELL-first step, `OutcomeHoldingSupervisor` also emits `OUTCOME_POST_FILL_QUALITY_SHADOW` at state change/ten seconds, with exact official-fill provenance, executable BBO return, and `SCRATCH_IOC_COUNTERFACTUAL`/`KEEP_NORMAL_EXIT_SHADOW`. It stops permanently after the official fill is 120 seconds old; long-held risk remains the responsibility of the existing bounded holding-path/crash observers. It has no stop-loss authority: it cannot cancel a protective SELL, alter a price or submit an IOC. The first two minutes and -3% executable loss are a watch bucket only; later P3/holding-path outcomes decide whether it would have been a false positive.
...
#### Research programme board and review gates (2026-09-15)

These streams deliberately run in parallel but cannot be promoted together: otherwise a later PnL change cannot be attributed to safety, entry execution, Deribit features, or the model.

| Stream | Collecting now | Five completed independent daily markets permits | Still blocks live promotion |
|---|---|---|---|
| Phase A/B entry quality | resting-order labels, official fills, P3 markout, two-minute post-fill path | first read-only adverse-selection review: count distinct resting orders, stale-cancel candidates, 5/10/30s markout and recovery outcomes | no live Phase-C entry canary; require >=30 distinct official maker BUY fills, >=10 distinct stale-cancel candidate orders with known later outcome, fee/depth-aware replay, and no material recover-winner harm across a predeclared unseen review set |
| Deribit D1–D3 | public perpetual/index snapshots, strict D2 as-of rows and daily-market walk-forward labels | rerun baseline-vs-perp walk-forward with real folds; decide whether the observed near-zero short-horizon contribution persists | a single market or many correlated snapshots are not independent evidence; any entry/exit model still needs 15–20 independent markets plus stable incremental executable value |
| Deribit D1.5 options | **not yet collected**; only planned ATM IV, 25-delta put/call skew and short-tenor term structure | if D1 uptime/freshness remains healthy across those five markets, implementation review may authorize a bounded public option-surface collector for tail-risk shadow | option features start as Market Risk Monitor/fair-value research only; they cannot choose a side, cancel, reprice or exit until a separate out-of-sample tail-versus-recovery test passes |
| E2/B1–B5 ML/fair-value | D2 features and labels, frozen shadow decisions and later realized/markout outcomes | first purged chronological walk-forward and feature-availability audit; reject weak features rather than tune live thresholds | five markets can identify obvious failure, not establish alpha; model artifact remains shadow-only pending 15–20 independent markets, fee/depth-aware executable targets and stable weekday/weekend/time-left results |

**Meaning of Phase C.** Five completed daily markets is a scheduled **review milestone**, not automatic entry into a live Phase-C canary. At that point run all reports, freeze the evaluation cohort and either reject a shadow action, continue collection, or write a separately authorized canary proposal. A live stale-BUY-cancel Phase C requires the first row's explicit entry-quality gates; dynamic active routing requires the stricter D4 gate. Option-surface collection may begin after the five-market D1 health review because it is passive data collection, not a Phase-C authority change.
...
### 2026-09-25 — Hyperliquid BTC Spot integration foundation (V1; separate from Outcome)

**Scope and identity.** Native BTC spot is a separate instrument and account-balance domain; it is not a different Outcome market period and must not inherit HIP-4 binary-price assumptions, Outcome lifecycle IDs, Outcome share sizing, or Outcome exit thresholds. `bot/hyperliquid_spot.py` resolves the BTC/USDC pair dynamically from `spotMeta`; it accepts the venue's documented `UBTC/USDC` naming as BTC and derives both the public coin `@{index}` and exchange asset ID `10000 + spot universe index`. Mainnet/testnet pair IDs are never constants. Price and size alignment use Hyperliquid's 5-significant-figure / spot `8 - szDecimals` price-decimal rule and metadata `szDecimals` size precision. Official references: [spot asset IDs](https://hyperliquid.gitbook.io/hyperliquid-docs/for-developers/api/asset-ids), [Info endpoint spot coin naming](https://hyperliquid.gitbook.io/hyperliquid-docs/for-developers/api/info-endpoint), and [tick and lot size](https://hyperliquid.gitbook.io/hyperliquid-docs/for-developers/api/tick-and-lot-size).

**Stage 1 — read-only shadow capture.** `BTC_SPOT_SHADOW_ENABLED=1` starts an isolated background worker with its own HTTP client. Every 5 seconds it records compact BTC/USDC top-three bid/ask levels, BBO/mid/spread, BTC perp all-mids, and (every 15 seconds) BTC perp mark/oracle/funding/OI/volume context to the already configured `OUTCOME_EXECUTION_JOURNAL_PATH`. It adds no database, no schema migration, no account read and no exchange mutation. Recorder writes are best-effort with a 50ms SQLite wait cap; on contention the observation may be skipped and the worker retries next interval, without holding up the Outcome execution loop. Capture does not interpolate gaps. It is opt-in and remains off unless the operator sets this flag in the local environment and restarts.

**Stage 2 — compare signals in BTC spot outcome space.** `python -m bot.btc_spot_shadow_report --db logs/outcome_shadow.db` reads only `BTC_SPOT_SHADOW_SNAPSHOT` rows and compares the sign of the preceding 5-minute spot return, perp-mid return, and fresh 5-minute perp OI change with the subsequent BTC spot-mid return over 5/15/60 minutes. Missing/stale context is excluded, never filled. This is a descriptive directional benchmark, not a calibrated terminal-probability model, execution simulation, or walk-forward promotion result; fewer than 10,000 snapshots or 15 UTC dates is explicitly `insufficient_data`. It has no dependency on Outcome admission / P3 labels and cannot write orders.

**Stage 2a — Outcome settlement-probability comparison (shadow only).** When `BTC_SPOT_SHADOW_ENABLED=1`, each existing `OutcomeResearchWorker` P2 capture obtains the latest already-captured BTC/USDC spot L2 mid history from `BTCSpotShadowCapture`; it makes no extra venue request and does not run on the wallet/execution lane. `OutcomeResearchCapture` writes a compact `OUTCOME_SETTLEMENT_PROBABILITY_SHADOW` event (and embeds the same evidence in the P2 snapshot) containing the market-implied YES/NO midpoints and a deterministic zero-drift lognormal probability from spot/strike, time remaining, and trailing one-hour realized variance. No interpolation is permitted; gaps over 60 seconds are excluded. A score is unavailable unless the current spot observation is at most 15 seconds old and history has at least 20 valid returns and 900 seconds of valid coverage. The shared pure estimator is `bot/outcome_settlement_probability.py`; `python -m bot.outcome_active_milestone_report --db logs/outcome_shadow.db --period 1d` includes the `settlement_probability_comparison` section, which scores live spot-shadow forecasts versus market midpoint on identical fixed time-left checkpoints after official settlement labels arrive. It remains `live_authority=false`, cannot change entry/exit decisions, and is not an execution-edge test. A missing/stale spot feed or insufficient warmup produces an explicit unavailable record, not an imputed probability. The underlying settlement comparator and lognormal/no-drift assumptions remain limitations; all model output is research evidence only.

**Stage 3 — isolated cash-spot execution adapter.** `HyperliquidSpotClient` and `BTCSpotExecutionService` do not use the HIP-4 order API or the Outcome SDK sidecar. Order mutation can only be constructed by `build_btc_spot_testnet_service(...)` when both `HL_TESTNET=1` (auth is actually testnet and URL host is exactly `api.hyperliquid-testnet.xyz`) and `BTC_SPOT_TESTNET_EXECUTION_ENABLED=1`; the production Outcome launcher never constructs this service and explicitly ignores this flag. This release has no BTC spot mainnet mutation implementation or strategy-to-spot route. Testnet service is ALO-only, cash spot (no leverage/borrow), one open BTC spot order, maximum $10 per order and $10 BTC position by default; every order requires fresh spot balance, open-order and timestamped L2 reads, non-crossing price, available USDC for buys / available UBTC for sells, a durable pre-mutation journal intent, a cloid, and exactly one fresh account-truth order match after submit. Cancel requires one exact coin/OID match and a fresh account snapshot proving OID absence. Unresolved durable spot intents are checked on service construction; unreadable journal state or any lost/ambiguous response, duplicate/missing match or failed durable record fences further mutations. These are testnet guardrails, not a certified recovery-complete mainnet execution system. The signed `/exchange` spot asset convention follows the [official Exchange endpoint](https://hyperliquid.gitbook.io/hyperliquid-docs/for-developers/api/exchange-endpoint).
