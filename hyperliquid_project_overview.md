...
**S0/E6 — 受限 OI live strategy 與時間型被動減損出場（現行語意校正 2026-08-31）：** S0 僅在 fresh Binance BTCUSDT public OI、同方向 Binance mark return／OI change、及 BTC spot 相對 Outcome strike 的方向確認同時成立時，才選 YES/NO；任何缺資料、過時資料、訊號衝突、價格上限使 fee-after target 不可行、或帳戶／stream gate 不安全都 `no-new-entry`。選定側 first-level maker bid <55¢ 時不可進場。出場先採 fee-after data-derived target（1%–5%，資料不足時 3%）；1h 後最多收窄至 +3%，2h 後最多收窄至 +2%。**僅在持倉已滿 2h 且 midpoint ≤ fill VWAP × 95% 時，現行實碼才允許 cancel-confirm-rebook 一張 fee-inclusive -5% ALO loss-band sell。** 它不是已成交的 stop，不會跨 bid、不能保證在 -5% 成交，也不會為 -20% gap 改成 taker；市場在 floor 以下時資金仍可被鎖至反彈或官方結算。S0 沒有持倉中的 OI/spot/mark reversal detector，也沒有止損後 cooldown；flat 後下一 entry 重新以 S0 當下條件決定 side。單筆／單 market 仍受 $11、單一 open order、ownership、freshness 與 ALO 限制，結果不得外推為 alpha 或擴大資金授權。

**S0/E6 實作與驗證（現行語意校正 2026-08-31）：** `OutcomeOiEntryGate` 的資料門檻不變。`launcher --live` 取得 typed `yes` 後會在 process 內啟用 official SDK execution、S0 與 E4 requote；E5 canary 仍預設關閉。`OUTCOME_LIVE_STRATEGY_ENTRY_PLACED` 寫入 target、1h/2h tier 及 `loss_reprice_pct="0.05"`。`_strategy_exit_tier()` 目前在 2h 後回傳 `(floor_return_pct, 0.05)`，因此 planner 會在 midpoint 跌逾 5% 時產生 -5% loss-band，不是 break-even；planner 的 replacement cap 為 `None`，但仍有 60 秒最小重掛間隔、tick hysteresis、ownership、account truth、fresh L2、ALO non-crossing 與 cancel-confirm-rebook 限制。舊文所稱 `loss_reprice_pct=0`、break-even-only、最多兩次 replacement、以及「預設 disabled」均為已淘汰的歷史狀態，不可用於解讀目前 live 行為。
...
**S0/E6 首張出場與 tier 修正（校正 2026-08-31）：** 首張 ALO sell 仍必定為 fee-after target；持倉未滿 1h 不得用 loss-band，1h 後僅可收窄至 +3%，2h 後僅可收窄至 +2%。先前本段把最終 tier 寫成 break-even-only、無負報酬止損與最多兩次 replacement，和目前 `_strategy_exit_tier()`／planner 不符，現已以本節與下方 S1 contract 的「2h + midpoint -5% → passive -5% loss-band」為準。此 loss-band 仍不使用 taker，也不是 guaranteed stop。
