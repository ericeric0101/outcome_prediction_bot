...
3. **P1-3 — WS freshness/gap fail-closed（校正 2026-08-30）。** `OutcomeStreamHealth` 要求：連線正常、連線後已完成 REST resync、Yes/No 兩側都收到 L2、且兩側都在 **15 秒**內新鮮。實盤 journal 顯示 Outcome quiet-book 的成對 L2 更新通常相隔約 5–6 秒；原本 3 秒門檻會把正常、仍連線的安靜盤面誤判為 `ws_book_stale`。15 秒容許兩個正常更新區間仍可下新 entry，但超過此時間、disconnect、reconnect、reconnect exhausted、market rollover、任一側 missing 仍 fail-closed。這不阻擋 cancel 或既有倉位的保護處置，避免資料故障反而使風險暴露無法縮小。launcher 會在市場 rollover 重建 recorder，並把狀態交給 official runtime。
