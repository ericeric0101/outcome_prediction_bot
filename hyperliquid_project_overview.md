...
**S1 final admission telemetry + pre-registered gate-ablation observer（完成 2026-09-03；不改變 live entry authority）。** 單有 `OUTCOME_ENTRY_GATE_DECISION` 只描述三方原始訊號，無法區分「訊號成立但被 WS、55¢ no-trade band、account/risk、target ceiling 或 existing order 擋下」與「根本沒有訊號」。現在每個 S0 tick 另寫 `OUTCOME_ENTRY_ADMISSION_DECISION`：raw signal/evidence、active account/recovery、market-data status、selected coin/best bid、no-trade/target/risk gate inputs、final state/reason、是否真的送單與 exact oid。它是 durable audit event，不會發出 exchange action。`OutcomeOiEntryGate` 同時把三個**預先註冊、telemetry-only** variants 寫入同一份 raw evidence：現行 baseline `spot ∧ 5m mark ∧ 5m OI`、候選 `spot ∧ 5m mark`、候選 `spot ∧ (5m mark OR 5m OI)`；OI 在 DOWN 時仍只代表 activity，方向仍由 spot/mark 決定。runtime 仍只會採用 baseline，variants 絕不授權 order。
...
9. **loss 後受限重進、平倉與重啟後：** official fills／account truth 先完成 FIFO reconciliation。managed loss exit 後先固定冷卻 15 分鐘；冷卻後只有一張受限 re-entry token，仍須重新通過當下 S0/Tier、fresh book、risk、fee-after target headroom。相同 side 還必須收復前次 official loss exit bid；不同 side 不自動反手，僅在當下完整新方向訊號成立時才可成為候選。official SDK 接受 resting BUY 後 token 才被消耗，該 daily market 的第二次 re-entry 仍拒絕到 rollover。沒有 loss exit 時，確認 flat 且不存在 managed order即可重新從當下 S0/Tier gate 選邊。重啟 recovery 只接管有 matching durable audit、market/coin、inventory 與 ownership 的 managed order；未知／手動訂單一律不接管。
...
**Confirmed narrow-IOC loss now enters the existing S2-3 gate.** A narrow hard-failure IOC that returns `emergency_exit_flat` now first synchronizes official `userFills`, then calls the existing `OutcomeLossReentryGate`. That gate independently requires the exact bot-owned BUY/SELL lot to reconcile as fee-inclusive negative PnL before writing `OUTCOME_LOSS_EXIT_CONFIRMED`. Only then does its fixed **15-minute** cooldown, same-coin reclaim of the official exit price, current S0/Tier/fresh-book/fee-after admission, and one-re-entry-per-market policy apply. This is an integration repair, not a new stop or a new operator setting. Partial/zero IOC fills, ambiguous ACKs, account/journal uncertainty, profitable exits, or an incomplete official lot do not record a loss and do not consume a re-entry token. It is intentionally not retroactive: historical exits remain forensic evidence, not newly fabricated lifecycle facts.
