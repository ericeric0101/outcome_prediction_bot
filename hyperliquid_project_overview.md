...
### S2 — reversal evidence, passive-loss audit, and re-entry discipline（計劃；未實作／未授權）
...
|---|---|---|
| **S2-0 — full holding-path telemetry** | 對每一個 confirmed inventory 以 event time 寫入 BBO/mid、executable exit VWAP、fill VWAP 比例、holding age、time-left、book health、OI/mark/spot-strike snapshot；持續到 actual close 或 official settlement。產生 MAE/MFE、-5% breach duration、passive loss quote 是否/何時成交、以及每筆最大持倉時間。 | 純資料；單元測試涵蓋 partial fill/restart/market rollover/no-lookahead。先補足「30 秒 markout 不代表整段 drawdown」缺口。 |
| **S2-1 — pure reversal classifier** | 新增不接 exchange 的 planner，輸入持倉時同一套 as-of OI/mark/spot-strike、book imbalance/health、持續時間及反向確認次數，輸出 `HOLD`、`WEAKENING`、`REVERSAL_CONFIRMED` 或 `UNKNOWN`。沒有資料、WS stale、訊號矛盾一律 `UNKNOWN/HOLD`。 | 只寫 journal 與 shadow counterfactual；必須以按 daily instance 切分、purged OOS 及實際持倉路徑證明，不得以單次 BTC move 直接選門檻。 |
| **S2-2 — bounded passive loss state machine** | 將現有 2h/-5% loss-band 變為明確 lifecycle：`LOSS_BAND_RESTING`、`LOSS_BAND_UNFILLED`、`REVERSAL_CONFIRMED`。每次改價皆 cancel-confirm-rebook，先處理 partial fill/cancel race，永遠 ALO。首版維持 -5% floor；若 book gap 在 floor 以下，只記錄 unfilled/breach duration並繼續保護，不假稱已止損。 | 在沒有新的 authority revision 與獨立 taker gate 前，禁止 marketable/taker，也禁止自動接受低於 -5% 的新 floor。 |
| **S2-3 — durable post-loss re-entry gate** | 僅在**實際 loss sell fill**得到 official `userFills` 證據後寫 `OUTCOME_LOSS_EXIT_CONFIRMED`；同 market/coin 的新 entry 必須讀此 evidence，先經冷卻期、fresh book、完整三方訊號及反轉分類器重新確認。它不能由「loss quote 已掛出」觸發，亦不得從 UI/PnL 推論。 | 初版先作 shadow/report，從 S2-0 的 recovery-time 分布校準 cooldown；在 OOS 前不得設定任意固定秒數並直接啟用 live。 |
